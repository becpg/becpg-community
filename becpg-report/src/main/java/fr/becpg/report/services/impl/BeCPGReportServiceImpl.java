...
public class BeCPGReportServiceImpl implements BeCPGReportService {
	...
	@Override
	public void generateReport(String templateId, String format, String lang, InputStream dataSource, OutputStream out, Map<String, byte[]> images)
			throws IOException {
		...
		try {
			...
			if (logger.isDebugEnabled()) {
				logger.debug("Birt IReportRunnable report generated in  " + ((System.currentTimeMillis() - startTime) / 1000.0) + " seconds ");
				startTime = System.currentTimeMillis();
			}
			...
			if (logger.isDebugEnabled()) {
				logger.debug("Birt report generated in  " + ((System.currentTimeMillis() - startTime) / 1000.0) + " seconds ");
			}
