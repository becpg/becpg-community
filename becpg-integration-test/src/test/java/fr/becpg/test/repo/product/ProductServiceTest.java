 ...
 * @author querephi
 */
public class ProductServiceTest extends PLMBaseTestCase {
	...
	public void testReportProduct() throws Exception {
		...
		final NodeRef rawMaterialNodeRef =	transactionService.getRetryingTransactionHelper().doInTransaction(new RetryingTransactionCallback<NodeRef>() {
			public NodeRef execute() throws Throwable {
				...
				NodeRef imagesNodeRef = fileFolderService.create(getTestFolderNodeRef(), TranslateHelper.getTranslatedPath(RepoConsts.PATH_IMAGES), ContentModel.TYPE_FOLDER).getNodeRef();
