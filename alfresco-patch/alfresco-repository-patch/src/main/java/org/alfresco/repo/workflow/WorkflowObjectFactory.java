...
public class WorkflowObjectFactory
{
	...
	public WorkflowTask createTask(String id, WorkflowTaskDefinition taskDef, String name, String defaultTitle,
			String defaultDescription, WorkflowTaskState state, WorkflowPath path, Map<QName, Serializable> properties) {
		return createTask(id, taskDef, name, defaultTitle, defaultDescription, state, path, path.getInstance()
				.getDefinition().getName(), properties);
	}

	public WorkflowTask createTask(String id, WorkflowTaskDefinition taskDef, String name, String defaultTitle,
			String defaultDescription, WorkflowTaskState state, WorkflowPath path, String workflowDefinitionName,
		...
		defaultDescription = (defaultDescription != null && defaultDescription.trim().length() == 0) ? null
				: defaultDescription;
		String description = getLabel(processKey, DESC_LABEL, defaultDescription,
				metadata.getDescription(), title);
