...
public class FrenchEcoScoreIT extends AbstractFinishedProductTest {
	...
	public void testFrenchEcoScore() {
		...
		transactionService.getRetryingTransactionHelper()
				.doInTransaction(() -> fillMaterial(packMaterial1, properties, 10d), false, true);
		...
		properties.put(PackModel.PROP_PM_ISNOTRECYCLABLE, false);
		transactionService.getRetryingTransactionHelper()
				.doInTransaction(() -> fillMaterial(packMaterial1, properties, 1d), false, true);

		final NodeRef finishedProduct1 = createFinishedProduct("Biscuit sec chocolaté, préemballé", "24036",
				"Prince goût chocolat", geoOrigin1, 48d, ing1, packMaterial1, null);

		inWriteTx(() -> {

			FinishedProductData finishedProductData = (FinishedProductData) alfrescoRepository
					.findOne(finishedProduct1);
		...
		transactionService.getRetryingTransactionHelper()
				.doInTransaction(() -> fillMaterial(packMaterial2, properties, 1d), false, true);
		...
		final NodeRef finishedProduct2 = createFinishedProduct(
				"Muesli croustillant au chocolat (non enrichi en vitamines et minéraux)", "32109",
				"Cruesli Chocolat noir", geoOrigin2, 37d, ing2, packMaterial2, null);

		transactionService.getRetryingTransactionHelper().doInTransaction(() -> {

			FinishedProductData finishedProductData = (FinishedProductData) alfrescoRepository
					.findOne(finishedProduct2);
		...
		properties.put(PackModel.PROP_PM_ISNOTRECYCLABLE, false);
		transactionService.getRetryingTransactionHelper()
				.doInTransaction(() -> fillMaterial(packMaterial3, properties, 1d), false, true);

		properties.clear();
		properties.put(ContentModel.PROP_NAME, "MSC");
		properties.put(PLMModel.PROP_LABEL_CLAIM_CODE, "MARINE_STEWARDSHIP_COUNCIL_LABEL");
		transactionService.getRetryingTransactionHelper()
				.doInTransaction(() -> fillLabelClaim(labelClaimList, properties), false, true);
		...
		final NodeRef finishedProduct3 = createFinishedProduct(
				"Sardine, filets sans arêtes à l'huile d'olive, appertisés, égouttés", "26231", "Sardines de Bretagne",
				geoOrigin2, 100d, ing2, packMaterial3, labelClaimList);

		transactionService.getRetryingTransactionHelper().doInTransaction(() -> {

			FinishedProductData finishedProductData = (FinishedProductData) alfrescoRepository
					.findOne(finishedProduct3);
	...
	private List<LabelClaimListDataItem> fillLabelClaim(List<LabelClaimListDataItem> labelClaimList,
			Map<QName, Serializable> properties) {

		NodeRef labelClaimNodeRef = nodeService
		...
		LabelClaimListDataItem labelClaimItem = new LabelClaimListDataItem(labelClaimNodeRef, "Test claim type",
				Boolean.TRUE);
	...
	private List<PackMaterialListDataItem> fillMaterial(List<PackMaterialListDataItem> packMaterial,
			Map<QName, Serializable> properties, double qty) {
		...
		/*
		 * packMaterial.add(new PackMaterialListDataItem(packMaterialNodeRef, qty, null,
		 * null, PackagingLevel.Primary));
		 */
		packMaterial.add(PackMaterialListDataItem.build().withMaterial(packMaterialNodeRef).withPerc(qty)
				.withPkgLevel(PackagingLevel.Primary));
		return packMaterial;
	}

	private NodeRef createFinishedProduct(String query, String categoryCode, String name, NodeRef geoOrigin,
			double ingQty, NodeRef ing, List<PackMaterialListDataItem> packMaterial,
		...
		NodeRef finishedProductNodeRef = inWriteTx(() -> {
			...
			/*
			 * ingList.add(new IngListDataItem(null, ingQty, Arrays.asList(geoOrigin), new
			 * ArrayList<>(), true, true, false, ing, false));
			 */
			ingList.add(IngListDataItem.build().withQtyPerc(ingQty).withgeoOrigin(Arrays.asList(geoOrigin))
					.withbioOrigin(new ArrayList<>()).withisGMO(true).withisIonized(true).withIngredient(ing)
					.withisManual(false));
