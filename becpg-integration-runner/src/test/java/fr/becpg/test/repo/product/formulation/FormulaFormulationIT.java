...
public class FormulaFormulationIT extends AbstractFinishedProductTest {
	...
	public void testAuthorizedTypes() {
		...
		NodeRef finishedProductDataNodeRef = inWriteTx(() -> {
			...
			dynamicCharactListItems.add(
					new DynamicCharactListItem("formula", "T(fr.becpg.repo.product.data.RawMaterialData).toString();"));
		...
		inReadTx(() -> {
			FinishedProductData finishedProductData = (FinishedProductData) alfrescoRepository
					.findOne(finishedProductDataNodeRef);
			DynamicCharactListItem dynamicCharactListItem = finishedProductData.getCompoListView()
					.getDynamicCharactList().get(0);
			assertEquals("class fr.becpg.repo.product.data.RawMaterialData",
					dynamicCharactListItem.getValue().toString());
			return null;
		});
		...
		inWriteTx(() -> {
			FinishedProductData finishedProductData = (FinishedProductData) alfrescoRepository
					.findOne(finishedProductDataNodeRef);
			List<DynamicCharactListItem> dynamicCharactList = finishedProductData.getCompoListView()
	...
	public void testCopyHelperFormula() throws Exception {
		...
		NodeRef finishedProductData1NodeRef = inWriteTx(() -> {
			...
			nodeService.createAssociation(finishedProductData1.getNodeRef(), client1.getNodeRef(),
		...
		inWriteTx(() -> {
			...
			assertEquals("Test Spel Formula 1",
					MLTextHelper.getClosestValue(finishedProductData2.getLegalName(), Locale.FRENCH));
			...
			assertEquals(1, associationService
					.getTargetAssocs(finishedProductData2.getNodeRef(), PLMModel.ASSOC_CLIENTS).size());
	...
	public void testRecursiveFormula() {
		NodeRef finishedProductDataNodeRef = inWriteTx(() -> {
			...
			/*
			 * SF1.getCompoListView().setCompoList(Arrays.asList(new CompoListDataItem(null,
			 * null, null, 1d, ProductUnit.kg, 0d, DeclarationType.Detail, sf2NodeRef)));
			 */
			SF1.getCompoListView()
					.setCompoList(Arrays.asList(CompoListDataItem.build().withQtyUsed(1d).withUnit(ProductUnit.kg)
			...
			/*
			 * SF2.getCompoListView().setCompoList(Arrays.asList(new CompoListDataItem(null,
			 * null, null, 1d, ProductUnit.kg, 0d, DeclarationType.Detail, sf1NodeRef)));
			 */
			SF2.getCompoListView()
					.setCompoList(Arrays.asList(CompoListDataItem.build().withQtyUsed(1d).withUnit(ProductUnit.kg)
							.withLossPerc(0d).withDeclarationType(DeclarationType.Detail).withProduct(sf1NodeRef)));
			...
			/*
			 * SF4.getCompoListView().setCompoList(Arrays.asList(new CompoListDataItem(null,
			 * null, null, 1d, ProductUnit.kg, 0d, DeclarationType.Detail, sf5NodeRef)));
			 */
			SF4.getCompoListView()
					.setCompoList(Arrays.asList(CompoListDataItem.build().withQtyUsed(1d).withUnit(ProductUnit.kg)
							.withLossPerc(0d).withDeclarationType(DeclarationType.Detail).withProduct(sf5NodeRef)));
			...
			/*
			 * compoList.add(new CompoListDataItem(null, null, null, 1d, ProductUnit.kg, 0d,
			 * DeclarationType.Detail, sf4NodeRef));
			 */
			...
			/*
			 * compoListFP.add(new CompoListDataItem(null, null, null, 1d, ProductUnit.kg,
			 * 0d, DeclarationType.Detail, sf1NodeRef));
			 */
			compoListFP.add(CompoListDataItem.build().withQtyUsed(1d).withUnit(ProductUnit.kg).withLossPerc(0d)
					.withDeclarationType(DeclarationType.Detail).withProduct(sf3NodeRef));
			/*
			 * compoListFP.add(new CompoListDataItem(null, null, null, 1d, ProductUnit.kg,
			 * 0d, DeclarationType.Detail, sf3NodeRef));
			 */
		...
		inWriteTx(() -> {
			...
			for (CompoListDataItem item : FP1.getCompoList()) {
				...
				Serializable dynCol = nodeService.getProperty(item.getNodeRef(),
