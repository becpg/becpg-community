...
public class MLTextHelperTest {
	...
	public void testIsSupportedLocale() {
		...
		try {
			...
			mlTextILL.addValue(MLTextHelper.parseLocale("en_US"),
					"keep in original u./ru	nopened packaging stored well closed in a cool dry place.");

			Assert.assertEquals(MLTextHelper.getClosestValue(mlTextILL, I18NUtil.getContentLocale()),
					"keep in original unopened packaging stored well closed in a cool dry place.");

			MLTextHelper.replaceTextForLanguage(I18NUtil.getContentLocale(), MLTextHelper.getClosestValue(mlTextILL, I18NUtil.getContentLocale()),
					mlTextILL);

			Assert.assertEquals(MLTextHelper.getClosestValue(mlTextILL, MLTextHelper.parseLocale("en")),
					"keep in original unopened packaging stored well closed in a cool dry place.");
			Assert.assertEquals(MLTextHelper.getClosestValue(mlTextILL, MLTextHelper.parseLocale("ii")),
					"keep in original unopened packaging stored well closed in a cool dry place.");
			Assert.assertEquals(MLTextHelper.getClosestValue(mlTextILL, MLTextHelper.parseLocale("")),
					"keep in original unopened packaging stored well closed in a cool dry place.");
			Assert.assertEquals(MLTextHelper.getClosestValue(mlTextILL, MLTextHelper.parseLocale("ar_EG")),
					"يحفظ في العبوة الأصلية غير المفتوحة المخزنة جيدًا في مكان بارد وجاف.");
