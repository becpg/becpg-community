...
public class FormulationCalcILWithYieldOnCompIT extends AbstractFinishedProductTest {
	 ...
	 * ingList (#685) RM2 has 200% Yield so water is added in a sub process but
	 * not displayed
	 * 
	 * @throws Exception
	 */
	...
	public void testCalculateIngListWithYieldOnComponents() throws Exception {
		...
		transactionService.getRetryingTransactionHelper().doInTransaction(() -> {
			...
			assertEquals(df.format(0.00375d), df.format(formulatedProduct.getCompoListView().getCompoList().get(2).getQty())); // 3.75g
																																// because
																																// of
																																// yield
																																// of
