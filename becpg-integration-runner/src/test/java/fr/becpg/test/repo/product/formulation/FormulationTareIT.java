...
public class FormulationTareIT extends AbstractFinishedProductTest {
	...
	public void testFormulationTare() throws Exception {
		...
		final NodeRef finishedProductNodeRef = transactionService.getRetryingTransactionHelper().doInTransaction(() -> {
			...
			compoList.add(new CompoListDataItem(null, null, null, 1d, ProductUnit.lb, 0d, DeclarationType.Declare, rawMaterial5NodeRef));// 9
																																			// *
																																			// 0.453592
																																			// /
																																			// 0.1
			...
			packList.add(new PackagingListDataItem(null, 0.2d, ProductUnit.L, PackagingLevel.Primary, true, packagingMaterial6NodeRef));// 0.2
																																		// but
																																		// was
																																		// 0
		...
		transactionService.getRetryingTransactionHelper().doInTransaction(() -> {
			...
			assertEquals(df.format(90d + 9d + 40.8233d + 2.5514d + 15d + (2 * 5d) + 3d + 50d + 28.349523125d + 0.5d + 0.2d),
					df.format(formulatedProduct.getTare()));
