...
public class CheckOutCheckInWebScriptTest extends BaseWebScriptTest{
	...
	/**
	 * Test check out check in product.
	 *
	...
	public void testCheckOutCheckInProduct() throws Exception {
		 ...
		 transactionService.getRetryingTransactionHelper().doInTransaction(new RetryingTransactionCallback<NodeRef>(){
			...
			String url = "/becpg/product/checkout/node/" + rawMaterialNodeRef.toString().replace(":/", "");
			String data = "";
			...
			logger.debug("content checkout: " + response.getContentAsString());
			 ...
			 transactionService.getRetryingTransactionHelper().doInTransaction(new RetryingTransactionCallback<NodeRef>(){
					...
					public NodeRef execute() throws Throwable {					   
						...
						List<FileInfo> files = fileFolderService.listFiles(tempFolder);
			 ...
			 url = "/becpg/product/checkin/node/" + workingCopyNodeRef.toString().replace(":/", "");			
			 logger.debug("url : " + url);				
	...
	/**
	 * Test check out cancel check out product.
	 *
	 * @throws Exception the exception
	 */
	public void testCheckOutCancelCheckOutProduct() throws Exception {
		 ...
		 transactionService.getRetryingTransactionHelper().doInTransaction(new RetryingTransactionCallback<NodeRef>(){
			...
			logger.debug("content checkout: " + response.getContentAsString());
			 ...
			 url = "/becpg/product/cancel-checkout/node/" + workingCopyNodeRef.toString().replace(":/", "");			
			 logger.debug("url : " + url);				
