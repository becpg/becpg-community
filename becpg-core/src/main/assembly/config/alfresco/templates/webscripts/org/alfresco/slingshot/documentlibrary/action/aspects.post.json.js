...
function runAction(p_params)
{
   ...
   try
   {
      ...
      for (i = 0, ii = removed.length; i < ii; i++)
      {
         ...
         {
			 aspectsRemoved[removed[i]] = {};
            assetNode.removeAspect(removed[i]);
            isTaggable = isTaggable || (removed[i] == "cm:taggable");
         }
      }
      
      if (Object.keys(aspectsRemoved).length > 0) {
	      bcpg.postEntityActivity(assetNode, "AspectsRemoval", "Update", JSON.stringify(aspectsRemoved));
	  }
      ...
      for (i = 0, ii = added.length; i < ii; i++)
      {
         ...
         {
			aspectsAdded[added[i]] = {};
            assetNode.addAspect(added[i]);
            isTaggable = isTaggable || (added[i] == "cm:taggable");
         }
      }
      
      if (Object.keys(aspectsAdded).length > 0) {
	      bcpg.postEntityActivity(assetNode, "AspectsAddition", "Update", JSON.stringify(aspectsAdded));
	  }
