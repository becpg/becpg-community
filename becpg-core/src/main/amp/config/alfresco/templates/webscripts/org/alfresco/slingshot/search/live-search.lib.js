...
function getEntityItem(container, node)
{
   var item =
         {
            nodeRef: node.nodeRef.toString(),
...
function liveSearch(params)
{
   switch (params.type)
   {
      case "documents":
         ...
         break;
      case "entities":
          return getEntityResults(params);
          break;   
      case "sites":
...
function getDocResults(params)
{
   ...
   // beCPG
   ftsQuery+= ' AND -TYPE:"bcpg:entityListItem" AND -TYPE:"rep:report" AND -ASPECT:"bcpg:hiddenFolder" ';
...
//beCPG
function getEntityResults(params)
{
   // ensure a TYPE is specified
   ...
   for (var i=0, item; i<nodes.length && i<params.maxResults; i++)
   {
      ...
      try
      {
         item = getEntityItem(splitQNamePath(nodes[i]), nodes[i]);
         if (item !== null)
         {
            results.push(item);
         }
