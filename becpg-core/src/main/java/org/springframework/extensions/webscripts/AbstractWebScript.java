...
public abstract class AbstractWebScript implements WebScript {
	...
	// Service resources
	final private Map<Locale, ResourceBundle> resources = new HashMap<>(4);
	final private ReadWriteLock resourcesLock = new ReentrantReadWriteLock();
	final private Map<String, String> jsonResources = new HashMap<>(4);
	final private ReadWriteLock jsonResourcesLock = new ReentrantReadWriteLock();

	// Status Template cache
	final private Map<String, StatusTemplate> statusTemplates = new HashMap<>(4);
	final private ReadWriteLock statusTemplateLock = new ReentrantReadWriteLock();

	// Script Context
	private String basePath;
	final private Map<String, ScriptDetails> scripts = new HashMap<>(4);
	final private ReadWriteLock scriptLock = new ReentrantReadWriteLock();

	/**
	 * <p>
	 * A lock to prevent concurrent modification of the module bundle cache.
	 * </p>
	 */
	...
	/**
	 * <p>
	 * A cache of {@link ResourceBundle} instances mapped against the path where
	 * they were found. {@link ModuleBundleSentinel} instances are cached
	 * against paths where no bundle could be found. This map is initialised
	 * with an initial size of 5 as it is not expected that there will be
	 * typically more many than this many extensions to a WebScript.
	 * </p>
	 */
	final private Map<String, ResourceBundle> moduleBundleCache = new HashMap<>(5);
	...
	public static final Class[] BECPG_CUSTOM_EXCEPTIONS;
	static {
		Class<?>[] coreClasses = new Class[] { SQLIntegrityConstraintViolationException.class };

		List<Class<?>> retryExceptions = new ArrayList<>();
		// Add core classes to the list.
		retryExceptions.addAll(Arrays.asList(coreClasses));

		BECPG_CUSTOM_EXCEPTIONS = retryExceptions.toArray(new Class[] {});
	};
	 ...
	 *            Parent container
	 * @param description
	 *            Description of the WebScript provided by the registry
	 */
	@Override
	public void init(Container container, Description description) {
		// sanity check to ensure a web script is only registered with a single
		// web script container
		if ((this.container != null) && (!this.container.equals(container))) {
			throw new WebScriptException(
					"WebScript " + description.getId() + " already associated with the '" + this.container.getName() + "' container");
		}
	...
	/**
	 * <p>
	 * Attempts to retrieve a previously cached {@link ResourceBundle} for the
	 * supplied path.
	 * </p>
	 * 
	 * @param path
	 *            The path to retrieve a cached bundle for.
	 * @return A previously cached bundle or <code>null</code> if it cannot be
	 *         found.
	 */
	...
	/**
	 * <p>
	 * Adds a new {@link ResourceBundle} into the cache mapped against the path
	 * that the bundle was found.
	 * </p>
	 * 
	 * @param path
	 *            The path to map the {@link ResourceBundle} against.
	 * @param bundle
	 *            The {@link ResourceBundle} to cache.
	 */
	...
	/**
	 * <p>
	 * Returns a {@link ResourceBundle} containing all the properties defined in
	 * the extension modules evaluated for a request. This method will cope with
	 * a base bundle having been found or not but the parameters must be
	 * supplied accordingly.
	 * </p>
	 * 
	 * @param container
	 *            This should be an object that implements the
	 *            {@link HandlesExtensibility} interface.
	 * @param result
	 *            This should be the base provided {@link ResourceBundle} but
	 *            can be <code>null</code> if a base {@link ResourceBundle} does
	 *            not exist.
	 * @param bundlePath
	 *            If the <code>result</code> parameter is <code>null</code>
	 *            (i.e. if no base {@link ResourceBundle} was found then this
	 *            needs to be a valid path to attempt to look for in the
	 *            extensions.
	 *            </p>
	 * @return A {@link ResourceBundle} containing properties merged from all
	 *         evaluated extension modules.
	 */
	private WebScriptPropertyResourceBundle getExtensionBundle(HandlesExtensibility container, ResourceBundle result, String bundlePath) {
		...
		if (container != null) {
			if (result instanceof WebScriptPropertyResourceBundle) {
				// If a base ResourceBundle was supplied then use it's resource
				// path (assuming
				// that it is a WebScriptPropertyResourceBundle, which it will
				// always be unless
				// the getBundleFromPath method has been overridden in the class
				// hierarchy.
				bundlePath = ((WebScriptPropertyResourceBundle) result).getResourcePath();
			}

			if (bundlePath != null) {
				// Get the current WebScript id and generate a new id from the
				// prefix (i.e. the package) and
				// the suffix (i.e. the bit after the package). Build a list of
				// locale paths for this new path
				// to allow for degrading of locale (e.g. from en_GB -> en ->
				// default).
				...
				// Iterate over the different locale paths in REVERSE order so
				// that the most specific locale
				// file is merged into the bundle last. This means that all
				// extension properties files will
				// be applied... this *could* mean a mixture of English with
				// other languages but should ensure
				// that no keys are displayed (unless the messages have
				// genuinely not been provided)...
				Object[] arrayOfPaths = paths.toArray();
				for (int i = arrayOfPaths.length - 1; i >= 0; i--) {
					String currPath = arrayOfPaths[i].toString();
					// If the bundle path is not null then iterate over the list
					// of possible files
					// that the evaluated modules suggest could provide
					// extensions to the current WebScript...
					for (String moduleBundlePath : container.getExtendingModuleFiles(currPath)) {
						try {
							// Check the cache to see if we've previously loaded
							// a bundle for this path...
							ResourceBundle moduleBundle = checkModuleBundleCache(moduleBundlePath);
							if (moduleBundle == null) {
								// If the cache does not contain a bundle mapped
								// against the path then
								// we know that it hasn't previously been
								// requested (if it had been requested
								// and couldn't be found then we'd have been
								// returned the sentinel).
								moduleBundle = getBundleFromPath(moduleBundlePath);
								if (moduleBundle == null) {
									// If a bundle truly doesn't exist for this
									// path (which is a perfectly
									// valid situation as modules do not have to
									// provide additional i18n
									// properties) then we should cache the
									// sentinel to ensure that we
									// don't needlessly look up the bundle
									// again...
									this.addModuleBundleToCache(moduleBundlePath, ModuleBundleSentinel.getInstance());
								} else {
									// If the bundle does exist then add it to
									// the cache...
									this.addModuleBundleToCache(moduleBundlePath, moduleBundle);
								}
							}

							// If we've found a module bundle and its NOT the
							// sentinel then we can process it...
							if ((moduleBundle != null) && (moduleBundle != ModuleBundleSentinel.getInstance())) {
								// If we have a bundle (regardless of whether or
								// not it has been retrieved from the
								// cache or whether we have just loaded it) we
								// need to add a record of its use for the
	...
	/**
	 * <p>
	 * Find the last index of the "/" and return 0 if it does not occur (rather
	 * than -1). The purpose of this method is exclusively for use with the
	 * String.substring method for finding the unqualified name of a WebScript.
	 * </p>
	 * 
	 * @param str
	 *            String
	...
	final public ResourceBundle getResources() {
		...
		// The bundle was not found in the cache for the requested locale so we
		// will attempt to
		// load it now and store it to the cache.
		if ((result == null) && !containsLocaleKey) {
			// If the bundle isn't already in the cache then attempt to load
			// it...
			try {
				// Create a list of all the paths to check/merge based on the
				// current locale and WebScript description.
				// We're then going to iterate BACKWARDS through the list so
				// that the most specific locales entries overwrite
				// the least specific.
				...
				for (int i = arrayOfPaths.length - 1; i >= 0; i--) {
					...
					if (currPathResult != null) {
						if (result == null) {
							...
							// Unless there have been significant overrides to
							// this class then we know that the
							// ResourceBundle will be an instance of
							// WebScriptPropertyResourceBundle. We are relying
							// on this being the case so that we can merge
				...
				// This code-block is intentionally performed before any
				// extension modules are processed.
				// In order to achieve optimum caching it has to be accepted
				// that an extension module cannot
				// change any import statements (although it can still override
				// imported properties)...
				if ((result != null) && (result instanceof WebScriptPropertyResourceBundle)) {
					try {
							...
							for (String includeBundle : includeBundles.toString().split("\\s*,\\s*")) {
								...
								for (int i = arrayOfImportPaths.length - 1; i >= 0; i--) {
									...
									if (currPathResult != null) {
										...
										// Unless there have been significant
										// overrides to this class then we know
										// that the
										// ResourceBundle will be an instance of
										// WebScriptPropertyResourceBundle. We
										// are relying
										// on this being the case so that we can
										// merge multiple properties files.
						...
						if (importedBundles != null) {
							...
							// importedBundles.merge(getDescription().toString(),
							// result);
				...
				// Write the results for the requested locale into the cache
				// (even if its null)...
				this.resourcesLock.writeLock().lock();
				try {
					// push the resources into the cache - null value is
					// acceptable if none found
		...
		if (container instanceof HandlesExtensibility) {
			// If the container handles extensibility then we need to apply
			// matching module extensions to
			// the bundle...
			...
			if (extendedBundle != null) {
				// The container has a previously cached bundle with extension
				// modules applied so we should
				// return that as the result.
				result = extendedBundle;
			} else {
				// The extension bundles have not previously been applied for
				// this WebScript in the current container so
				// we need to apply them now and then ask the container to cache
				// them. In reality the container should only
				// be caching for each request, i.e. resource bundle extensions
				// will be needed to be generated per request
				// but ONLY once...
				//
				// PLEASE NOTE: There is no thread locking used here as it is
				// perfectly acceptable for multiple threads to
				// apply extension modules at the same time. This is because
				// each request can apply different
				// modules. What is important is that this happens ONLY once.
				WebScriptPropertyResourceBundle extensionBundle = getExtensionBundle((HandlesExtensibility) container, result,
						this.getDescription().getId());
	...
	/**
	 * <p>
	 * A locale based lookup sequence is build using the supplied {@link Locale}
	 * and (if it is different) the default {@link Locale}.
	 * <ol>
	 * <li>Lookup
	 * <{@code}descid><{@code}language_country_variant>.properties</li>
	 * <li>Lookup <{@code}descid><{@code}language_country>.properties</li>
	 * <li>Lookup <{@code}descid><{@code}language>.properties</li>
	 * </ol>
	 * The repeat but with the default {@link Locale}. Finally lookup
	 * <descid>.properties
	 * </p>
	 * 
	 * @param path
	...
	private LinkedHashSet<String> buildLocalePathList(final String path, final Locale locale) {
		final LinkedHashSet<String> pathSet = new LinkedHashSet<>();
		...
		if (locale.equals(Locale.getDefault())) {
			// We're already using the default Locale, so don't add it's paths
			// again.
		} else {
	 ...
	 * 
	 * @param description
	 *            The current {@link WebScript} {@link Description}.
	 * @param locale
	 *            The requested {@link Locale}.
	 * @return LinkedHashSet<String>
	 */
	...
	protected ScriptDetails getExecuteScript(String mimetype) {
		...
		if (script == null) {
			...
			if (scriptContent != null) {
				// Validate that there is actually a reader registered to handle
				// this format
			...
			if (logger.isDebugEnabled()) {
				logger.debug("Caching script " + ((script == null) ? "null" : script.getContent().getPathDescription()) + " for web script "
						+ basePath + " and request mimetype " + ((mimetype == null) ? "null" : mimetype));
			}
	 ...
	 * @return script model
	 */
	protected Map<String, Object> createScriptParameters(WebScriptRequest req, WebScriptResponse res, ScriptDetails script,
			Map<String, Object> customParams) {
		Map<String, Object> params = new HashMap<>(32, 1.0f);
	...
	protected Map<String, Object> createTemplateParameters(WebScriptRequest req, WebScriptResponse res, Map<String, Object> customParams) {
		Map<String, Object> params = new HashMap<>(64, 1.0f);
		...
		params.put("message", getMessageMethod()); // for compatibility with
													// repo templates
		params.put("msg", getMessageMethod()); // short form for presentation
												// webscripts
	...
	final protected Map<String, String> createArgs(WebScriptRequest req) {
		...
		Map<String, String> args = new HashMap<>(names.length, 1.0f);
	...
	final protected void renderTemplate(String templatePath, Map<String, Object> model, Writer writer) {
		...
		if (validTemplatePath != null) {
			...
			if ((container instanceof HandlesExtensibility) && !((HandlesExtensibility) container).isExtensibilitySuppressed()) {
				...
				// Get the extensibility model from the container, add some
				// unboundContent to handle any output
				// prior to the first extensibility FreeMarker directive and
				...
				// Process the template as normal, but sent the output to the
				// model writer which stores the content
				// in a buffer so that we can apply the any extending modules to
				// it before flushing the final content
				...
				// Switch into extension processing (this will prevent modules
				// writing directly into the model,
				// they will need to use actions to update the model)...
				extModel.switchToExtensionProcessing();

				// Get the module handler from the container and retrieve any
				// templates that provide extensions to the
	 ...
	 *            output writer
	 * @param extension
	 *            optional template extension type (i.e. ftl, php)
	 */
	...
	/**
	 * Create an exception whose associated message is driven from a status
	 * template and model
	 * 
	 * @param e
	 *            exception
	...
	final protected WebScriptException createStatusException(Throwable e, final WebScriptRequest req, final WebScriptResponse res) {
		// Unwrap the exception in case there is an underlying
		// WebScriptException
		// Look for JavaScriptException specifically thrown by a JS
		// implementation
		Throwable current = e;
		do {
			if (current instanceof WebScriptException) {
				...
				// extract "code" and "message" properties from JavaScript error
				// object
				...
				if (val instanceof ScriptableObject) {
					...
					// found an error object - look for code and message
					// properties
					...
					if (unwrapped instanceof Map) {
						...
						// beCPG
						for (Class<?> clazz : RetryingTransactionHelper.RETRY_EXCEPTIONS) {
							if (msg.contains(clazz.getName())) {
								throw new ConcurrencyFailureException(msg);
							}
						}

						// beCPG
						for (Class<?> clazz : BECPG_CUSTOM_EXCEPTIONS) {
							if (msg.contains(clazz.getName())) {
								throw new ConcurrencyFailureException(msg);
							}
						}
						...
						// See
						// http://www.mozilla.org/rhino/ScriptingJava.html#Exceptions
						// A caught / rethrown error object has the Java
						// exception in a javaException property
		...
		if (current == null) {
			we = new WebScriptException(HttpServletResponse.SC_INTERNAL_SERVER_ERROR, "Wrapped Exception (with status template): " + e.getMessage(),
					e);
		} else {
	...
	protected StatusTemplate getStatusTemplate(String scriptId, int statusCode, String format) {
		...
		if (statusTemplate == null) {
			...
			if (logger.isDebugEnabled()) {
				logger.debug("Caching template " + statusTemplate.getPath() + " for web script " + scriptId + " and status " + statusCode
						+ " (format: " + format + ")");
			}
	...
	/**
	 * Looks for the script's config file and reads it's contents if present.
	 * The result is the XML config stored in the <code>xmlConfig</code> member
	 * variable.
	 */
	private synchronized void setupScriptConfig() {
		if (!this.xmlConfigInitialised) {
			...
			try {
				...
				if (input != null) {
					...
					try (BufferedReader reader = new BufferedReader(new InputStreamReader(input, "UTF-8"), 1024)) {
				...
				// Indicate that we at least attempted to retrieve the XML
				// configuration (even if an exception
				// was thrown in the process) so that we don't try again...
				this.xmlConfigInitialised = true;
			}
		}

		// Check to see if the container handles extensibility and if it does
		// then attempt to create the Script and Template
		// configuration models using it. These versions will contain the
		// default configuration plus additional configuration
		// that is dynamically provided by modules evaluated to be applied to
		// the current invocation of this WebScript
		...
		// Check to see if the Script and Template configuration models were set
		// up and if not, create them from the default
	...
	protected void executeScript(ScriptContent location, Map<String, Object> model) {
		...
		if (container instanceof HandlesExtensibility) {
			// Get the module handler from the container and retrieve any
			// scripts that provide extensions to the
			// the current WebScript...
	 ...
	 * 
	 * @param resources
	 *            To render - can be null if no resources present, in which case
	 *            an empty JSON object will be output.
	 * 
	...
	private String renderJSONResources(ResourceBundle resources) {
		...
		if (resources != null) {
			...
			if (resources instanceof WebScriptPropertyResourceBundle) {
				// Add a String of all the additional paths merged into the
				// bundle to ensure that we do not
				// retrieve stale cached data. It is important that we always
				// return bundle data that is
				// specific to the request as each request might result in
				// different modules being applied
