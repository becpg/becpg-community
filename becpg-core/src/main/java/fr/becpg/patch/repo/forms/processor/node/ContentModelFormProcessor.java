 ...
 */
package fr.becpg.patch.repo.forms.processor.node;
            ...
            FilteredFormProcessor<ItemType, PersistType>
{
                ...
                List<AbstractAssocCommand> assocCommands)
    {
        ...
        if (m.matches())
        {
            ...
            if (assocDef == null)
            {
            	//beCPG patch, we want to force the add of association if it is defined in model
            	if (getLogger().isDebugEnabled())
            		getLogger().debug("Patch, try to get the assocDef '" + fieldName + "', fullQName '" + fullQName + "'.");
            	assocDef = dictionaryService.getAssociation(fullQName);            	
            	if(assocDef == null)
            	{
