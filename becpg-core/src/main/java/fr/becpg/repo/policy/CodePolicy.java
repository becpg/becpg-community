...
public class CodePolicy implements NodeServicePolicies.OnAddAspectPolicy {
	...
	public void onAddAspect(NodeRef nodeRef, QName type) {
		...
		if(generateCode){
			...
			code = autoNumService.getAutoNumValue(typeQName.getLocalName(), BeCPGModel.PROP_CODE.getLocalName());
			nodeService.setProperty(nodeRef, BeCPGModel.PROP_CODE, code);
		}
		else{
			// store autoNum in db
			autoNumService.createOrUpdateAutoNumValue(typeQName.getLocalName(), BeCPGModel.PROP_CODE.getLocalName(), code);
		}		
	}	
}
