			...
			if(wUsedList == null){
	    		...
	    		{    				    				    			
		    		...
		    		if(filesToUpdate.contains(wUsedListDataItem.getNodeRef())){
		    			...
		    			nodeService.createAssociation(childAssocRef.getChildRef(), wUsedListDataItem.getChangeUnit(), ECOModel.ASSOC_WUL_CHANGE_UNIT);		    			
		    		}			    					    				    		
	    		}
			}
		}
	...
	private List<WUsedListDataItem> loadWUsedList(NodeRef listContainerNodeRef){
    	...
    	{    		
    		...
    		{    			
	    		...
	    		for(NodeRef nodeRef : nodeRefs)
		    	{	
		    		List<AssociationRef> changeUnitAssocRefs = nodeService.getTargetAssocs(nodeRef, ECOModel.ASSOC_WUL_CHANGE_UNIT);
		    		NodeRef changeUnitNodeRef = changeUnitAssocRefs.isEmpty() ? null : (changeUnitAssocRefs.get(0)).getTargetRef();		    		
