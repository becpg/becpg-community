...
public class ProductDAOImpl implements ProductDAO {
	...
	public List<? extends SimpleListDataItem> loadSimpleList(NodeRef productNodeRef, QName dataList) {
		...
		if (dataList.equals(BeCPGModel.TYPE_COSTLIST)) {
			...
			return loadPhysicoChemList(listsContainerNodeRef);
		} else {
			logger.debug(String.format("DataList '%s' is not loaded since it is not implemented.", dataList));
		}
		return null;
	...
	public List<AllergenListDataItem> loadAllergenList(NodeRef listContainerNodeRef) {
		...
		if (listContainerNodeRef != null) {
			...
			if (allergenListNodeRef != null) {
				...
				List<NodeRef> listItemNodeRefs = entityListDAO.getListItems(allergenListNodeRef, BeCPGModel.TYPE_ALLERGENLIST);
	...
	public List<DynamicCharactListItem> loadDynamicCharactList(NodeRef listContainerNodeRef) {
		...
		if (listContainerNodeRef != null) {
			...
			dynamicCharactListNodeRef = entityListDAO.getList(listContainerNodeRef, BeCPGModel.TYPE_PACKAGINGLIST);
			if (dynamicCharactListNodeRef != null) {
				List<NodeRef> listItemNodeRefs = entityListDAO.getListItems(dynamicCharactListNodeRef, BeCPGModel.TYPE_DYNAMICCHARACTLIST);
				for (NodeRef listItemNodeRef : listItemNodeRefs) {
					dynamicCharactList.add(loadDynamicCharactListItem(listItemNodeRef));
			...
			dynamicCharactListNodeRef = entityListDAO.getList(listContainerNodeRef, MPMModel.TYPE_PROCESSLIST);
			if (dynamicCharactListNodeRef != null) {
				List<NodeRef> listItemNodeRefs = entityListDAO.getListItems(dynamicCharactListNodeRef, BeCPGModel.TYPE_DYNAMICCHARACTLIST);
				for (NodeRef listItemNodeRef : listItemNodeRefs) {
					dynamicCharactList.add(loadDynamicCharactListItem(listItemNodeRef));
	...
	private List<CompoListDataItem> loadCompoList(NodeRef listContainerNodeRef) {
		...
		if (listContainerNodeRef != null) {
			...
			if (compoListNodeRef != null) {
				...
				List<NodeRef> listItemNodeRefs = entityListDAO.getListItems(compoListNodeRef, BeCPGModel.TYPE_COMPOLIST);
	...
	public List<CostListDataItem> loadCostList(NodeRef listContainerNodeRef) {
		...
		if (listContainerNodeRef != null) {
			...
			if (costListNodeRef != null) {
				...
				List<NodeRef> listItemNodeRefs = entityListDAO.getListItems(costListNodeRef, BeCPGModel.TYPE_COSTLIST);
	...
	private List<PriceListDataItem> loadPriceList(NodeRef listContainerNodeRef) {
		...
		if (listContainerNodeRef != null) {
			...
			if (priceListNodeRef != null) {
				...
				List<NodeRef> listItemNodeRefs = entityListDAO.getListItems(priceListNodeRef, BeCPGModel.TYPE_PRICELIST);
	...
	public List<IngListDataItem> loadIngList(NodeRef listContainerNodeRef) {
		...
		if (listContainerNodeRef != null) {
			...
			if (ingListNodeRef != null) {
				...
				List<NodeRef> listItemNodeRefs = entityListDAO.getListItems(ingListNodeRef, BeCPGModel.TYPE_INGLIST);
	...
	public List<NutListDataItem> loadNutList(NodeRef listContainerNodeRef) {
		...
		if (listContainerNodeRef != null) {
			...
			if (nutListNodeRef != null) {
				...
				List<NodeRef> listItemNodeRefs = entityListDAO.getListItems(nutListNodeRef, BeCPGModel.TYPE_NUTLIST);
	...
	private List<OrganoListDataItem> loadOrganoList(NodeRef listContainerNodeRef) {
		...
		if (listContainerNodeRef != null) {
			...
			if (organoListNodeRef != null) {
				...
				List<NodeRef> listItemNodeRefs = entityListDAO.getListItems(organoListNodeRef, BeCPGModel.TYPE_ORGANOLIST);
	...
	public List<IngLabelingListDataItem> loadIngLabelingList(NodeRef listContainerNodeRef) {
		...
		if (listContainerNodeRef != null) {
			...
			if (ingLabelingListNodeRef != null) {
				...
				List<NodeRef> listItemNodeRefs = entityListDAO.getListItems(ingLabelingListNodeRef, BeCPGModel.TYPE_INGLABELINGLIST);
	...
	private List<MicrobioListDataItem> loadMicrobioList(NodeRef listContainerNodeRef) {
		...
		if (listContainerNodeRef != null) {
			...
			if (microbioListNodeRef != null) {
				...
				List<NodeRef> listItemNodeRefs = entityListDAO.getListItems(microbioListNodeRef, BeCPGModel.TYPE_MICROBIOLIST);
	...
	private List<PhysicoChemListDataItem> loadPhysicoChemList(NodeRef listContainerNodeRef) {
		...
		if (listContainerNodeRef != null) {
			...
			if (physicoChemListNodeRef != null) {
				...
				List<NodeRef> listItemNodeRefs = entityListDAO.getListItems(physicoChemListNodeRef, BeCPGModel.TYPE_PHYSICOCHEMLIST);
	...
	private List<PackagingListDataItem> loadPackagingList(NodeRef listContainerNodeRef) {
		...
		if (listContainerNodeRef != null) {
			...
			if (pkgingListNodeRef != null) {
				...
				List<NodeRef> listItemNodeRefs = entityListDAO.getListItems(pkgingListNodeRef, BeCPGModel.TYPE_PACKAGINGLIST);
	...
	private List<ForbiddenIngListDataItem> loadForbiddenIngList(NodeRef listContainerNodeRef) {
		...
		if (listContainerNodeRef != null) {
			...
			if (forbiddenIngListNodeRef != null) {
				...
				List<NodeRef> listItemNodeRefs = entityListDAO.getListItems(forbiddenIngListNodeRef, BeCPGModel.TYPE_FORBIDDENINGLIST);
	...
	private List<ReqCtrlListDataItem> loadReqCtrlList(NodeRef listContainerNodeRef) {
		...
		if (listContainerNodeRef != null) {
			...
			if (reqCtrlListNodeRef != null) {
				...
				List<NodeRef> listItemNodeRefs = entityListDAO.getListItems(reqCtrlListNodeRef, BeCPGModel.TYPE_REQCTRLLIST);
	...
	private List<ProcessListDataItem> loadProcessList(NodeRef listContainerNodeRef) {
		...
		if (listContainerNodeRef != null) {
			...
			if (processListNodeRef != null) {
				...
				List<NodeRef> listItemNodeRefs = entityListDAO.getListItems(processListNodeRef, MPMModel.TYPE_PROCESSLIST);
			...
			if (allergenList == null) {
				...
				for (AllergenListDataItem allergenListDataItem : allergenList) {
					...
					if (filesToUpdate.containsKey(allergenNodeRef)) {
						...
						nodeService.createAssociation(linkNodeRef, allergenListDataItem.getAllergen(), BeCPGModel.ASSOC_ALLERGENLIST_ALLERGEN);
					}
	...
	private void createDynamicCharactList(NodeRef listContainerNodeRef, List<DynamicCharactListItem> dynamicCharactList) {
		if (listContainerNodeRef != null) {
			NodeRef compoDynamicCharactListNodeRef = entityListDAO.getList(listContainerNodeRef, BeCPGModel.TYPE_COMPOLIST);
...
//			 dynamicCharact list, create if needed
			if (compoDynamicCharactListNodeRef == null) {
				compoDynamicCharactListNodeRef = entityListDAO.createList(listContainerNodeRef, BeCPGModel.TYPE_COMPOLIST);
			}
			
//		NO delete possible without looking into 3 dataLists
//			
//			
//
//			// Concat 3 lists
//			List<NodeRef> listItemNodeRefs = entityListDAO.getListItems(compoDynamicCharactListNodeRef, BeCPGModel.TYPE_DYNAMICCHARACTLIST);
//			
//
//			// create temp list
...
//			}
			...
			for (DynamicCharactListItem dynamicCharactListDataItem : dynamicCharactList) {
				...
				if (linkNodeRef!=null) {
					...
					// create
					ChildAssociationRef childAssocRef = nodeService.createNode(compoDynamicCharactListNodeRef, ContentModel.ASSOC_CONTAINS,
		...
		for (AbstractComponent<CompoListDataItem> component : composite.getChildren()) {
			...
			if (filesToUpdate.contains(compoListDataItem.getNodeRef())) {
				...
				ChildAssociationRef childAssocRef = nodeService.createNode(compoListNodeRef, ContentModel.ASSOC_CONTAINS,
						QName.createQName(NamespaceService.CONTENT_MODEL_1_0_URI, GUID.generate()), BeCPGModel.TYPE_COMPOLIST, properties);
	...
	public void createCostList(NodeRef listContainerNodeRef, List<CostListDataItem> costList) {
		if (listContainerNodeRef != null) {
			...
			if (costList == null) {
				...
				// update or create nodes
				int sortIndex = RepoConsts.SORT_DEFAULT_STEP; // sort by
																// formulation
				for (CostListDataItem costListDataItem : costList) {
		...
		if (listContainerNodeRef != null) {
			...
			if (priceList == null) {
				...
				for (PriceListDataItem priceListDataItem : priceList) {
					...
					properties.put(BeCPGModel.PROP_START_EFFECTIVITY, priceListDataItem.getStartEffectivity());
					properties.put(BeCPGModel.PROP_END_EFFECTIVITY, priceListDataItem.getEndEffectivity());
					
					if (filesToUpdate.contains(priceListDataItem.getNodeRef())) {
						// update
						nodeService.addProperties(priceListDataItem.getNodeRef(), properties);
					} else {
						// create
			...
			if (organoList == null) {
				...
				for (OrganoListDataItem organoListDataItem : organoList) {
					...
					if (filesToUpdate.containsKey(organoNodeRef)) {
						// update
						nodeService.addProperties(filesToUpdate.get(organoNodeRef), properties);
					} else {
						// create
	...
	private void createIngLabelingList(NodeRef listContainerNodeRef, List<IngLabelingListDataItem> ingLabelingList) throws InvalidTypeException {
		...
		if (listContainerNodeRef != null) {
			...
			if (ingLabelingList == null) {
				...
				List<NodeRef> listItemNodeRefs = entityListDAO.getListItems(illNodeRef, BeCPGModel.TYPE_INGLABELINGLIST);
				...
				for (IngLabelingListDataItem illDataItem : ingLabelingList) {
					...
					if (filesToUpdate.containsKey(grpNodeRef)) {
						// update
						nodeService.addProperties(filesToUpdate.get(grpNodeRef), properties);
						illDataItem.setNodeRef(filesToUpdate.get(grpNodeRef));
					} else {
						// create
						ChildAssociationRef childAssocRef = nodeService.createNode(illNodeRef, ContentModel.ASSOC_CONTAINS,
								QName.createQName(NamespaceService.CONTENT_MODEL_1_0_URI, GUID.generate()), BeCPGModel.TYPE_INGLABELINGLIST, properties);
			...
			if (microbioList == null) {
				...
				for (MicrobioListDataItem microbioListDataItem : microbioList) {
					...
					if (filesToUpdate.containsKey(microbioNodeRef)) {
						// update
						nodeService.addProperties(filesToUpdate.get(microbioNodeRef), properties);
					} else {
						// create
			...
			if (physicoChemList == null) {
				...
				for (PhysicoChemListDataItem physicoChemListDataItem : physicoChemList) {
					...
					if (filesToUpdate.containsKey(physicoChemNodeRef)) {
						// update
						nodeService.addProperties(filesToUpdate.get(physicoChemNodeRef), properties);
					} else {
						// create
	...
	private void createPackagingList(NodeRef listContainerNodeRef, List<PackagingListDataItem> packagingList) throws InvalidTypeException {
		...
		if (listContainerNodeRef != null) {
			...
			if (packagingList == null) {
				...
				List<NodeRef> listItemNodeRefs = entityListDAO.getListItems(packagingListNodeRef, BeCPGModel.TYPE_PACKAGINGLIST);
				...
				for (PackagingListDataItem packagingListDataItem : packagingList) {
					...
					if (filesToUpdate.contains(packagingListDataItem.getNodeRef())) {
						// update
						nodeService.addProperties(packagingListDataItem.getNodeRef(), properties);
					} else {
						// create
