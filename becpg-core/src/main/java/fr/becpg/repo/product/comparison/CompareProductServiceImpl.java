...
/**
 * Compare several products (properties, datalists and composite datalists).
 *
 * @author querephi
 */
public class CompareProductServiceImpl implements CompareProductService {
	...
	/** The logger. */
	private static Log logger = LogFactory.getLog(CompareProductServiceImpl.class);
	...
	/* (non-Javadoc)
	 * @see fr.becpg.repo.product.comparison.CompareProductService#compare(org.alfresco.service.cmr.repository.NodeRef, java.util.List)
	 */
	@Override
	public List<CompareResultDataItem> compare(NodeRef product1,
			List<NodeRef> products) {
		
		Map<String, CompareResultDataItem> comparisonMap = new HashMap<String, CompareResultDataItem>();		
		int pos = 1;
		int nbProducts = products.size() + 1;		
		
		for(NodeRef product : products){
			compareProducts(product1, product, nbProducts,  pos, comparisonMap);
			pos++;
		}
	...
	/* (non-Javadoc)
	 * @see fr.becpg.repo.product.comparison.CompareProductService#compareStructDatalist(org.alfresco.service.cmr.repository.NodeRef, org.alfresco.service.cmr.repository.NodeRef, org.alfresco.service.namespace.QName, org.alfresco.service.namespace.QName)
	 */
	@Override
	public List<StructCompareResultDataItem> compareStructDatalist(NodeRef product1NodeRef, NodeRef product2NodeRef, QName datalistType, QName pivotProperty) {
		...
		NodeRef listsContainer1NodeRef = nodeService.getChildByName(product1NodeRef, BeCPGModel.ASSOC_PRODUCTLISTS, RepoConsts.CONTAINER_DATALISTS);
		if(listsContainer1NodeRef != null){
			NodeRef list1NodeRef = nodeService.getChildByName(listsContainer1NodeRef, ContentModel.ASSOC_CONTAINS, datalistType.getLocalName());
			
			if(list1NodeRef != null)
				dataListItems1 = fileFolderService.listFiles(list1NodeRef);
		}
		
		NodeRef listsContainer2NodeRef = nodeService.getChildByName(product2NodeRef, BeCPGModel.ASSOC_PRODUCTLISTS, RepoConsts.CONTAINER_DATALISTS);
		if(listsContainer2NodeRef != null){
			NodeRef list2NodeRef = nodeService.getChildByName(listsContainer2NodeRef, ContentModel.ASSOC_CONTAINS, datalistType.getLocalName());
	 ...
	 * Compare 2 products.
	 *
	 * @param product1NodeRef the product1 node ref
	 * @param product2NodeRef the product2 node ref
	 * @param nbProducts the nb products
	 * @param comparisonPosition the comparison position
	 * @param comparisonMap the comparison map
	 */
	private void compareProducts(NodeRef product1NodeRef, NodeRef product2NodeRef, int nbProducts, int comparisonPosition, Map<String, CompareResultDataItem> comparisonMap){
		
		//compare product properties		
		compareNode(null, null, null, product1NodeRef, product2NodeRef, nbProducts, comparisonPosition, false, comparisonMap);
		
		//load datalists
		List<String> comparedDataLists = new ArrayList<String>();
		List<FileInfo> dataLists1 = new ArrayList<FileInfo>();    	
		NodeRef listsContainer1NodeRef = nodeService.getChildByName(product1NodeRef, BeCPGModel.ASSOC_PRODUCTLISTS, RepoConsts.CONTAINER_DATALISTS);
		if(listsContainer1NodeRef != null){
			dataLists1 = fileFolderService.listFolders(listsContainer1NodeRef);
		}
		
		List<FileInfo> dataLists2 = new ArrayList<FileInfo>();		
		NodeRef listsContainer2NodeRef = nodeService.getChildByName(product2NodeRef, BeCPGModel.ASSOC_PRODUCTLISTS, RepoConsts.CONTAINER_DATALISTS);
		if(listsContainer2NodeRef != null){
			dataLists2 = fileFolderService.listFolders(listsContainer2NodeRef);
		}
    	
    	
    	for(FileInfo dataList1 : dataLists1){
    		    		
    		String dataListType1 = (String)nodeService.getProperty(dataList1.getNodeRef(), DataListModel.PROP_DATALISTITEMTYPE);
    		comparedDataLists.add(dataListType1);
    		
    		// look for dataList2
			NodeRef dataList2NodeRef = null;
			if(dataLists2 != null){
				for(FileInfo d : dataLists2){
					String dataListType2 = (String)nodeService.getProperty(d.getNodeRef(), DataListModel.PROP_DATALISTITEMTYPE);
					if(dataListType1.equals(dataListType2)){
						dataList2NodeRef = d.getNodeRef();
					}
				}
			}
			...
			compareDataLists(QName.createQName(dataListType1, namespaceService), dataList1.getNodeRef(), dataList2NodeRef, nbProducts, comparisonPosition, comparisonMap);
    	}
    	
    	//compare dataLists2 that have not been compared
    	for(FileInfo dataList2 : dataLists2){
    		
    		String dataListType2 = (String)nodeService.getProperty(dataList2.getNodeRef(), DataListModel.PROP_DATALISTITEMTYPE);
			...
			compareDataLists(QName.createQName(dataListType2, namespaceService), null, dataList2.getNodeRef(), nbProducts, comparisonPosition, comparisonMap);
    	}
	}
	
	/**
	 * Compare 2 product data lists.
	 *
	 ...
	 * @param comparisonMap the comparison map
	 */
	private void compareDataLists(QName productList, NodeRef dataList1NodeRef, NodeRef dataList2NodeRef, int nbProducts, int comparisonPosition, Map<String, CompareResultDataItem> comparisonMap){
		
		QName pivotProperty = null;
		boolean isCompositeDL = false;
		
		//look for pivot
		if(productList.getLocalName().equals(BeCPGModel.TYPE_ALLERGENLIST.getLocalName())){
			pivotProperty = BeCPGModel.PROP_ALLERGENLIST_ALLERGEN;
		}
		else if(productList.getLocalName().equals(BeCPGModel.TYPE_COMPOLIST.getLocalName())){
			isCompositeDL = true;
			pivotProperty = BeCPGModel.ASSOC_COMPOLIST_PRODUCT;
		}
		else if(productList.getLocalName().equals(BeCPGModel.TYPE_COSTLIST.getLocalName())){
			pivotProperty = BeCPGModel.ASSOC_COSTLIST_COST;
		}
		else if(productList.getLocalName().equals(BeCPGModel.TYPE_INGLABELINGLIST.getLocalName())){
			//TODO
			return;
			//pivotProperty = BeCPGModel.PROP_ILL_GRP;
		}
		else if(productList.getLocalName().equals(BeCPGModel.TYPE_INGLIST.getLocalName())){
			pivotProperty = BeCPGModel.ASSOC_INGLIST_ING;
		}
		else if(productList.getLocalName().equals(BeCPGModel.TYPE_MICROBIOLIST.getLocalName())){
			pivotProperty = BeCPGModel.ASSOC_MICROBIOLIST_MICROBIO;
		}
		else if(productList.getLocalName().equals(BeCPGModel.TYPE_NUTLIST.getLocalName())){
			pivotProperty = BeCPGModel.ASSOC_NUTLIST_NUT;
		}
		else if(productList.getLocalName().equals(BeCPGModel.TYPE_ORGANOLIST.getLocalName())){
			pivotProperty = BeCPGModel.ASSOC_ORGANOLIST_ORGANO;
		}
		else if(productList.getLocalName().equals(BeCPGModel.TYPE_PHYSICOCHEMLIST.getLocalName())){
			pivotProperty = BeCPGModel.ASSOC_PHYSICOCHEMLIST_PHYSICOCHEM;
		}				
		else{
			//TODO : specific productLists ? Not implemented
			return;
		}
		...
		// look for characteristics that are in both product
		...
		for(CharacteristicToCompare c : characteristicsToCmp){
			compareNode(productList, c.getCharactPath(), c.getCharacteristic(), c.getNodeRef1(), c.getNodeRef2(), nbProducts, comparisonPosition, true, comparisonMap);
		}
	}
	 ...
	 *
	 * @param productListType the product list type
	 * @param pivotProperty the pivot property
	 * @param strucComparisonList the struc comparison list
	 * @param compositeItem1 the composite item1
	 * @param compositeItem2 the composite item2
	 */
	private void structCompareCompositeDataLists(QName productListType, QName pivotProperty, List<StructCompareResultDataItem> strucComparisonList, CompositeComparableItem compositeItem1, CompositeComparableItem compositeItem2){
		...
		if(compositeItem1 != null){
			for(String key : compositeItem1.getItemList().keySet()){
				...
				Map<String, CompareResultDataItem> comparisonMap = new HashMap<String, CompareResultDataItem>();
				compareNode(productListType, null, null, nodeRef1, nodeRef2, 2, 1, true, comparisonMap);
				...
				StructCompareResultDataItem structComparison = new StructCompareResultDataItem(productListType, c1.getDepthLevel(), operator, nodeRef1, nodeRef2, properties1, properties2);
				...
				structCompareCompositeDataLists(productListType, pivotProperty, strucComparisonList, tempCompositeItem1, tempCompositeItem2);
			}
		}
		...
		if(compositeItem2 != null){
			for(String key : compositeItem2.getItemList().keySet()){
				...
				if((compositeItem1 == null) || (compositeItem1 != null && compositeItem1.get(key) == null)){
					...
					// get Properties
					Map<String, CompareResultDataItem> comparisonMap = new HashMap<String, CompareResultDataItem>();
					compareNode(productListType, null, null, null, c2.getNodeRef(), 2, 1, true, comparisonMap);
					Map<QName, String> properties1 = new HashMap<QName, String>();
					Map<QName, String> properties2 = new HashMap<QName, String>();
					for(String key2 : comparisonMap.keySet()){											
					...
					StructCompareResultDataItem structComparison = new StructCompareResultDataItem(productListType, c2.getDepthLevel(), StructCompareOperator.Added, null, c2.getNodeRef(), properties1, properties2);
					strucComparisonList.add(structComparison);
					
					if(c2 instanceof CompositeComparableItem){
						structCompareCompositeDataLists(productListType, pivotProperty, strucComparisonList, null, (CompositeComparableItem)c2);
					}
				}			
			}
		}
	 ...
	 * @param comparisonMap the comparison map
	 */
	private void compareNode(QName productList, List<NodeRef> charactPath, NodeRef characteristic, NodeRef nodeRef1, NodeRef nodeRef2, int nbProducts, int comparisonPosition, boolean isDataList, Map<String, CompareResultDataItem> comparisonMap){
		...
		for(QName propertyQName : properties1.keySet()){
			...
			if(isCompareableProperty(propertyQName, isDataList)){
				...
				compareValues(productList, charactPath, characteristic, propertyQName, oValue1, oValue2, nbProducts, comparisonPosition, comparisonMap, propertyFormats);
			}						
		}
		...
		for(QName propertyQName : properties2.keySet()){
			...
			if(!properties1.containsKey(propertyQName) && isCompareableProperty(propertyQName, isDataList)){
				compareValues(productList, charactPath, characteristic, propertyQName, null,  properties2.get(propertyQName), nbProducts, comparisonPosition, comparisonMap, propertyFormats);
			}
		}		
		...
		for(QName propertyQName : associations1Sorted.keySet()){
			...
			if(isCompareableProperty(propertyQName, isDataList)){
				...
				if(isDifferent){
					compareAssocs(productList, charactPath, characteristic, propertyQName, nodeRefs1, nodeRefs2, nbProducts, comparisonPosition, comparisonMap);							
				}
			}						
		}
		...
		for(QName propertyQName : associations2Sorted.keySet()){
			...
			if(!associations1Sorted.containsKey(propertyQName) && isCompareableProperty(propertyQName, isDataList)){
				compareAssocs(productList, charactPath, characteristic, propertyQName, null,  associations2Sorted.get(propertyQName), nbProducts, comparisonPosition, comparisonMap);
			}
		}
	 ...
	 * @param comparisonMap the comparison map
	 */
	private void compareAssocs(QName productList, List<NodeRef> charactPath, NodeRef characteristic, QName propertyQName, List<NodeRef> nodeRefs1, List<NodeRef> nodeRefs2, int nbProducts, int comparisonPosition, Map<String, CompareResultDataItem> comparisonMap){
		...
		String key = String.format("%s-%s-%s", productList, characteristic, propertyQName);		
		CompareResultDataItem comparisonDataItem = comparisonMap.get(key);
		
		if(comparisonDataItem == null){
			List<String> values = new ArrayList<String>(nbProducts);
			values.add(0, strValue1);
			values.add(comparisonPosition, strValue2);
			comparisonDataItem = new CompareResultDataItem(productList, charactPath, characteristic, propertyQName, values);
	 ...
	 * @param comparisonMap the comparison map
	 */
	private void compareValues(QName productList, List<NodeRef> charactPath, NodeRef characteristic, QName propertyQName, Serializable oValue1, Serializable oValue2, int nbProducts, int comparisonPosition, Map<String, CompareResultDataItem> comparisonMap, PropertyFormats propertyFormats){
		...
		String key = String.format("%s-%s-%s", productList, characteristic, propertyDef.getName());	
		CompareResultDataItem comparisonDataItem = comparisonMap.get(key);
