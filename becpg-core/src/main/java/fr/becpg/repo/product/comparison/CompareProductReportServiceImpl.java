...
/**
 * The Class CompareProductReportServiceImpl.
 *
 * @author querephi
 */
public class CompareProductReportServiceImpl  implements CompareProductReportService{
	
	//comparisonRows
	/** The Constant TAG_PRODUCTS_COMPARISON. */
	private static final String TAG_PRODUCTS_COMPARISON = "productsComparison";
	...
	/** The Constant ATTR_PRODUCT_1. */
	private static final String ATTR_PRODUCT_1 = "product1";
	
	/** The Constant ATTR_PRODUCT_2. */
	private static final String ATTR_PRODUCT_2 = "product2";
	
	/** The Constant ATTR_PRODUCT_3. */
	private static final String ATTR_PRODUCT_3 = "product3";
	
	/** The Constant ATTR_PRODUCT_4. */
	private static final String ATTR_PRODUCT_4 = "product4";		
	
	/** The Constant ATTR_PRODUCTLIST. */
	private static final String ATTR_PRODUCTLIST = "productList";
	...
	/**
	 * Sets the compare product service.
	 *
	 * @param compareProductService the new compare product service
	 */
	public void setCompareProductService(CompareProductService compareProductService) {
	...
	/* (non-Javadoc)
	 * @see fr.becpg.repo.product.comparison.CompareProductReportService#getComparisonReport(org.alfresco.service.cmr.repository.NodeRef, java.util.List, java.io.OutputStream)
	 */
	@Override
	public OutputStream getComparisonReport(NodeRef product1, List<NodeRef> products, OutputStream outputStream){				
		...
		if(templateNodeRef != null){
			...
			// do comparison
			List<CompareResultDataItem> compareResult = compareProductService.compare(product1, products);
			...
			for(NodeRef productNodeRef : products){
				String comparison = (String)nodeService.getProperty(product1, ContentModel.PROP_NAME) + COMPARISON_SEPARATOR + (String)nodeService.getProperty(productNodeRef, ContentModel.PROP_NAME);
				List<StructCompareResultDataItem> scr = compareProductService.compareStructDatalist(product1, productNodeRef, BeCPGModel.TYPE_COMPOLIST, BeCPGModel.ASSOC_COMPOLIST_PRODUCT);				
			...
			Element productsCmpElt = document.addElement(TAG_PRODUCTS_COMPARISON);
			productsCmpElt.add(renderComparisonAsXmlData(product1, products, compareResult));
			productsCmpElt.add(renderStructComparisonAsXmlData(structCompareResults, BeCPGModel.ASSOC_COMPOLIST_PRODUCT));			
			...
			try{
				...
				ByteArrayInputStream bais = new ByteArrayInputStream( productsCmpElt.asXML().getBytes());
...
//				//DEBUG code
//				FileOutputStream fosXML = new FileOutputStream(new File("/tmp/cmpProducts_written.xml"));
//				OutputFormat format = OutputFormat.createPrettyPrint();
//				XMLWriter writer = new XMLWriter(fosXML, format);
//				writer.write(productsCmpElt.getDocument());
//				writer.flush();
//				
//				FileOutputStream fos = new FileOutputStream(new File("/tmp/cmpProducts_written.pdf"));
...
//				// End DEBUG code
	 ...
	 * @param productsNodeRef the products node ref
	 * @param compareResult the compare result
	 * @return the element
	 */
	private Element renderComparisonAsXmlData(NodeRef product1NodeRef, List<NodeRef> productsNodeRef, List<CompareResultDataItem> compareResult){
			
			Document document = DocumentHelper.createDocument();
			
			Element cmpRowsElt = document.addElement(TAG_COMPARISON_ROWS);
			
			String name = (String)nodeService.getProperty(product1NodeRef, ContentModel.PROP_NAME);			
			cmpRowsElt.addAttribute(ATTR_PRODUCT_1, name);
			
			name = (String)nodeService.getProperty(productsNodeRef.get(0), ContentModel.PROP_NAME);
			cmpRowsElt.addAttribute(ATTR_PRODUCT_2, name);
			
			if(productsNodeRef.size() > 2){
				name = (String)nodeService.getProperty(productsNodeRef.get(1), ContentModel.PROP_NAME);
				cmpRowsElt.addAttribute(ATTR_PRODUCT_3, name);
			}
				
			if(productsNodeRef.size() > 3){
				name = (String)nodeService.getProperty(productsNodeRef.get(2), ContentModel.PROP_NAME);
				cmpRowsElt.addAttribute(ATTR_PRODUCT_4, name);
			}
			...
			for(CompareResultDataItem c : compareResult){
				...
				cmpRowElt.addAttribute(ATTR_PRODUCTLIST, productListTitle);
			...
			for(String comparison : structCompareResults.keySet()){
				...
				for(StructCompareResultDataItem c : structCompareResult){
					...
					if(c.getCharacteristic1() != null){
			    		...
			    		product1 = (String)nodeService.getProperty(productNodeRef, ContentModel.PROP_NAME);				    		
					}
					...
					if(c.getCharacteristic2() != null){
			    		...
			    		NodeRef productNodeRef = (compoAssocRefs.get(0)).getTargetRef();
			    		product2 = (String)nodeService.getProperty(productNodeRef, ContentModel.PROP_NAME);				    		
					}		
					...
					cmpRowElt.addAttribute(ATTR_ITEM1, product1);
					cmpRowElt.addAttribute(ATTR_ITEM2, product2);
	...
	private NodeRef getComparisonTemplate() {
        ...
        sp.setQuery(RepoConsts.PATH_QUERY_REPORT_COMPARE_PRODUCTS);	        
