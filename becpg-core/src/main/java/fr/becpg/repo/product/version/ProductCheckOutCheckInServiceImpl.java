 ...
 * @author querephi
 */
public class ProductCheckOutCheckInServiceImpl extends CheckOutCheckInServiceImpl {
	...
	/** The logger. */
	private static Log logger = LogFactory.getLog(ProductCheckOutCheckInServiceImpl.class);
	...
	public NodeRef checkout(
        ...
        final NodeRef workingCopy = AuthenticationUtil.runAs(new AuthenticationUtil.RunAsWork<NodeRef>()
        {
			...
			public NodeRef doWork() throws Exception
            {
                ...
                //do not incremet productCode
                autoNumService.decreaseAutoNumValue(BeCPGModel.TYPE_PRODUCT.getLocalName(), BeCPGModel.PROP_PRODUCT_CODE.getLocalName());
        ...
        try
        {
            ...
            // Reset productCode as productCode of checked out version
            this.nodeService.setProperty(workingCopy, BeCPGModel.PROP_PRODUCT_CODE, this.nodeService.getProperty(nodeRef, BeCPGModel.PROP_PRODUCT_CODE));
        ...
        // Copy product datalists
		// Rights are checked by copyService during recursiveCopy
		AuthenticationUtil.runAs(new AuthenticationUtil.RunAsWork<Void>(){
            @Override
			public Void doWork() throws Exception
            {      
            	productService.copyProductLists(nodeRef, workingCopy, true);
            	return null;
	...
	public NodeRef checkin(final NodeRef workingCopyNodeRef, Map<String, Serializable> properties) {		
        ...
        if (nodeService.hasAspect(workingCopyNodeRef, ContentModel.ASPECT_COPIEDFROM) == true){        	
            ...
            productNodeRef = (NodeRef) workingCopyProperties.get(ContentModel.PROP_COPY_REFERENCE);
            ...
            //copy properties
    		copyService.copy(workingCopyNodeRef, productNodeRef);
    		
    		//create new version
            productVersionService.createVersion(productNodeRef, properties);
    		
            logger.debug("workingCopyNodeRef.versionLabel: " + nodeService.getProperty(workingCopyNodeRef, BeCPGModel.PROP_VERSION_LABEL));
            logger.debug("productNodeRef.versionLabel: " + nodeService.getProperty(productNodeRef, BeCPGModel.PROP_VERSION_LABEL));                		
    		
    		// Copy product datalists
    		// Rights are checked by copyService during recursiveCopy
    		final NodeRef finalProductNodeRef = productNodeRef;
    		AuthenticationUtil.runAs(new AuthenticationUtil.RunAsWork<Void>(){
                @Override
				public Void doWork() throws Exception
                {      
                	productService.copyProductLists(workingCopyNodeRef, finalProductNodeRef, true);
                	return null;
            ...
            this.nodeService.removeAspect(productNodeRef, ContentModel.ASPECT_LOCKABLE);                        
        }
        else{
