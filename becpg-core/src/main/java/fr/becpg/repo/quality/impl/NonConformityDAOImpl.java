...
public class NonConformityDAOImpl implements BeCPGDao<NonConformityData> {
	...
	public NodeRef create(NodeRef parentNodeRef, NonConformityData ncData) {
		...
		NodeRef cpNodeRef = nodeService.createNode(parentNodeRef, ContentModel.ASSOC_CONTAINS, 
								QName.createQName(NamespaceService.CONTENT_MODEL_1_0_URI, ncData.getName()), 
								QualityModel.TYPE_NC, properties).getChildRef();
    	...
    	{    		
    		...
    		if(workLogNodeRef != null)
    		{
		    	...
		    	{	    			
		    		...
		    		WorkLogDataItem workLogDataItem = new WorkLogDataItem(nodeRef, 
		    							(String)nodeService.getProperty(nodeRef, QualityModel.PROP_WL_STATE),
		    							(String)nodeService.getProperty(nodeRef, QualityModel.PROP_WL_COMMENT));
