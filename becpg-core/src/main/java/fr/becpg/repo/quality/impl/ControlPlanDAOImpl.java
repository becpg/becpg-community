...
public class ControlPlanDAOImpl implements BeCPGDao<ControlPlanData> {
	...
	public NodeRef create(NodeRef parentNodeRef, ControlPlanData cpData) {
		...
		NodeRef cpNodeRef = nodeService.createNode(parentNodeRef, ContentModel.ASSOC_CONTAINS,
				QName.createQName(NamespaceService.CONTENT_MODEL_1_0_URI, QName.createValidLocalName(cpData.getName())), QualityModel.TYPE_CONTROL_PLAN, properties).getChildRef();
		...
		if (listContainerNodeRef != null) {
			...
			if (samplingDefListNodeRef != null) {
				...
				for (int z_idx = 0; z_idx < nodes.size(); z_idx++) {
					...
					SamplingDefListDataItem samplingDefListDataItem = new SamplingDefListDataItem(nodeRef, (Integer) nodeService.getProperty(nodeRef, QualityModel.PROP_SDL_QTY),
							(Integer) nodeService.getProperty(nodeRef, QualityModel.PROP_SDL_FREQ), (String) nodeService.getProperty(nodeRef, QualityModel.PROP_SDL_FREQUNIT),
							controlPointNodeRef, controlStepNodeRef, controlingGroupNodeRef);
				...
				for (SamplingDefListDataItem samplingDefListDataItem : samplingDefList) {
					...
					if (filesToUpdate.containsKey(samplingDefNodeRef)) {
						...
						ChildAssociationRef childAssocRef = nodeService.createNode(samplingDefListNodeRef, ContentModel.ASSOC_CONTAINS,
								QName.createQName(NamespaceService.CONTENT_MODEL_1_0_URI, GUID.generate()), QualityModel.TYPE_SAMPLINGDEF_LIST, properties);
