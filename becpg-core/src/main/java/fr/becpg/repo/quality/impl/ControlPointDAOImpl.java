...
public class ControlPointDAOImpl implements BeCPGDao<ControlPointData> {
	...
	public NodeRef create(NodeRef parentNodeRef, ControlPointData cpData) {
		...
		NodeRef cpNodeRef = nodeService.createNode(parentNodeRef, ContentModel.ASSOC_CONTAINS,
				QName.createQName(NamespaceService.CONTENT_MODEL_1_0_URI, QName.createValidLocalName(cpData.getName())), QualityModel.TYPE_CONTROL_POINT, properties).getChildRef();
			...
			if (controlDefList == null) {
				...
				for (ControlDefListDataItem controlDefListDataItem : controlDefList) {
					...
					if (filesToUpdate.containsKey(controlDefNodeRef)) {
						...
						// create
						ChildAssociationRef childAssocRef = nodeService.createNode(controlDefListNodeRef, ContentModel.ASSOC_CONTAINS,
								QName.createQName(NamespaceService.CONTENT_MODEL_1_0_URI, GUID.generate()), QualityModel.TYPE_CONTROLDEF_LIST, properties);
