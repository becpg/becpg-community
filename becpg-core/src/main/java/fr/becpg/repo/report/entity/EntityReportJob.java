...
import fr.becpg.repo.batch.NodeRefErrorHandlingBatchStep;
...
public class EntityReportJob extends AbstractScheduledLockedJob implements Job {
	...
	public void executeJob(JobExecutionContext context) throws JobExecutionException {
		...
		beCPGCacheService = (BeCPGCacheService) jobData.get("beCPGCacheService");
	...
	private int generatePendingReports(NodeService nodeService, EntityVersionService entityVersionService, EntityReportService entityReportService, BatchQueueService batchQueueService, BatchPriority priority, int maxResults) {
		...
		if (priority != null) {
			pendingNodes.addAll(BeCPGQueryBuilder.createQuery()
					...
					.excludeProp(BeCPGModel.PROP_BATCH_ERROR_IDS, batchFullId)
					.inDBIfPossible().list());
		...
		BatchStep<NodeRef> batchStep = new NodeRefErrorHandlingBatchStep(nodeService, beCPGCacheService, batchInfo) {
			@Override
			public void processEntry(NodeRef entityNodeRef) throws Throwable {
