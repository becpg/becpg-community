...
public class DataListSortServiceImpl implements DataListSortService {
	...
	private void fixSortableList(QName dataType, NodeRef listContainer) {
		...
		List<NodeRef> listItems = BeCPGQueryBuilder.createQuery().parent(listContainer).isNotNull(BeCPGModel.PROP_SORT).addSort(BeCPGModel.PROP_SORT, true).list();
	...
	public void move(NodeRef nodeRef, boolean moveUp) {
		...
		if (destNodeRef == null) {
			...
			BeCPGQueryBuilder queryBuilder = BeCPGQueryBuilder.createQuery()
					.parent(listContainer)
					.andBetween(BeCPGModel.PROP_SORT, String.valueOf(sort + 1), String.valueOf(lastSort))
					.ofType(dataType)
			...
			queryBuilder = BeCPGQueryBuilder.createQuery()
					.parent(listContainer)
					.andBetween(BeCPGModel.PROP_SORT, String.valueOf(destSort + 1), String.valueOf(lastDestSort))
					.ofType(dataType)
					.addSort(BeCPGModel.PROP_SORT, true);
