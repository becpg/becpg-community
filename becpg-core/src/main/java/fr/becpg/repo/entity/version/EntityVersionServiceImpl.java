...
/**
 * Store the entity version history in the SpacesStore otherwise we cannot use lucene query and datalists don't work so we cannot get them.
 *
 * @author querephi
 */
public class EntityVersionServiceImpl implements EntityVersionService {	
	 ...
	 * @return the node ref
	 */
	private NodeRef createVersion(final NodeRef versionHistoryNodeRef, final NodeRef entityNodeRef, VersionNumber versionNumber, String versionDescription){						
		...
		try{
			...
			versionNodeRef = AuthenticationUtil.runAs(new AuthenticationUtil.RunAsWork<NodeRef>(){
	            ...
	            {      
	            	...
	            	return copyService.copy(entityNodeRef, versionHistoryNodeRef, ContentModel.ASSOC_CONTAINS, ContentModel.ASSOC_CHILDREN	, true);
			...
			versionProperties.put(BeCPGModel.PROP_FROZEN_NODE_DBID, nodeService.getProperty(entityNodeRef, ContentModel.PROP_NODE_DBID));
    ...
    {      
    	...
    	if(getEntitysHistoryFolder() != null){
    		vhNodeRef = nodeService.getChildByName(getEntitysHistoryFolder(), ContentModel.ASSOC_CONTAINS, nodeRef.getId());
    	}
