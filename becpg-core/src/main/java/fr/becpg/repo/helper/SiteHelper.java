...
public class SiteHelper {
		...
		if (path.contains(SITES_SPACE_QNAME_PATH)) {
			...
			if (pos >= 1) {
				...
				if (pos >= 1) {
					...
					containerId = containerId.substring(containerId
							.indexOf(":") + 1);
	...
	public static String extractSiteId(String prefixedPath) {
		String siteId = null;
		if (prefixedPath.contains(SITES_SPACE_QNAME_PATH)) {

			String tmp = prefixedPath
					.substring(SITES_SPACE_QNAME_PATH.length());
			int pos = tmp.indexOf(':');
			if (pos > 0) {
				siteId = tmp.split("/")[0];
