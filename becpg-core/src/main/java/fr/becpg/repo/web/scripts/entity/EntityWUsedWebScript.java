...
/**
 * The Class EntityWUsedWebScript.
 *
 * @author querephi
 */
public class EntityWUsedWebScript extends DeclarativeWebScript  {
	...
	/** The logger. */
	private static Log logger = LogFactory.getLog(EntityWUsedWebScript.class);
	...
	/**
	 * Return the entity where useds
	 * 
	 * url : /becpg/entity/wused/node/{store_type}/{store_id}/{id}.
	 *
	 * @param req the req
	...
	protected Map<String, Object> executeImpl(WebScriptRequest req, Status status, Cache cache){
		...
		logger.debug("entityWUsedWebScript executeImpl()");
			
		NodeRef entityNodeRef = new NodeRef(storeType, storeId, nodeId);
		
		QName wusedType = productDictionaryService.getWUsedList(entityNodeRef);
		...
		if(BeCPGModel.TYPE_COMPOLIST.equals(wusedType)){
			...
			List<CompoListDataItem> wUsedList = productService.getWUsedCompoList(entityNodeRef);
			...
			List<PackagingListDataItem> wUsedList = productService.getWUsedPackagingList(entityNodeRef);
			model.put(MODEL_KEY_NAME_TOTALRECORDS, wUsedList.size());
			model.put(MODEL_KEY_NAME_WUSEDITEMS, wUsedList);
		}
