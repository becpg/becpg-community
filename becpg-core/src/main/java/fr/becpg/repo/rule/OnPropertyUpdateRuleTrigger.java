...
public class OnPropertyUpdateRuleTrigger extends RuleTriggerAbstractBase implements NodeServicePolicies.OnUpdatePropertiesPolicy {
	...
	*/
	...
	public void registerRuleTrigger() {
		// Bind behaviour
		this.policyComponent.bindClassBehaviour(QName.createQName(NamespaceService.ALFRESCO_URI, "onUpdateProperties"), this,
	...
	private boolean havePropertiesBeenModified(NodeRef nodeRef, Map<QName, Serializable> before, Map<QName, Serializable> after, boolean newNode,
		...
		Set<QName> keys = new HashSet<>(after.keySet());
		...
		for (QName name : keys) {
			...
			// Ignore protected properties
				...
				if (propertyDefinition.getDataType().getName().equals(DataTypeDefinition.CONTENT) && !propertyDefinition.isMultiValued()) {
					...
					if (this.ignoreEmptyContent) {
						...
						if (!ContentData.hasContent(beforeContent) || (beforeContent!=null &&  beforeContent.getSize() == 0)) {
							beforeValue = null;
						}
						if (!ContentData.hasContent(afterContent) || (afterContent!=null &&  afterContent.getSize() == 0)) {
							afterValue = null;
						}
					}
					if (newNode) {
	...
	private static ContentData toContentData(Object object) {
		...
		if (object instanceof ContentData contentData) {
			return contentData;
		}
	...
	public void onUpdateProperties(NodeRef nodeRef, Map<QName, Serializable> before, Map<QName, Serializable> after) {
		...
		if (before.size() != 0 && // ALF-4846: Do not trigger for newly created nodes	
	...
	private void triggerRules(NodeRef nodeRef) {
		if (triggerParentRules ) {
			...
			for (ChildAssociationRef parentAssocRef : parentsAssocRefs) {
				...
				if (logger.isDebugEnabled()) {
					logger.debug("OnPropertyUpdate rule triggered (parent); " + "nodeRef=" + parentAssocRef.getParentRef());
				}
			}
		} else {
