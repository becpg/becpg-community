	...
	AuthenticationUtil.runAs(new RunAsWork()
         {
     		public Object doWork() throws Exception
             {
     		   ...
     		   SearchParameters sp = new SearchParameters();
     		    ...
     		    sp.setQuery("+TYPE:\"bcpg:entity\" -ASPECT:\"bcpg:effectivityAspect\"");	
		    ...
		    print("migration size: " + result.getNodeRefs().size());
     		    ...
     		    for(NodeRef nodeRef : result.getNodeRefs()){
     		    	print("Add aspect: "+nodeRef);
			nodeService.addAspect(nodeRef, BeCPGModel.ASPECT_EFFECTIVITY, null);     		    	
     		    }
     			return null;
             }
         }, systemUserName);
...
for (Tenant tenant : tenantService.getAllTenants()){
     	migrate(tenantService.getDomainUser(AuthenticationUtil.getSystemUserName(), tenant.getTenantDomain()));
}
