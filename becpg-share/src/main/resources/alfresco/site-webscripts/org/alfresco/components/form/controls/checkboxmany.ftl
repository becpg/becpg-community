...
<div class="form-field checkboxmany">
   <#if form.mode == "view">
               ...
               <#list field.control.params.options?split(optionSeparator) as nameValue>
                <#if nameValue?? && nameValue!= "" && nameValue!= "|">
                  <#if nameValue?index_of(labelSeparator) == -1>
                   <p> <input  onchange="javascript:beCPG.util.updateMultiCheckboxesValue('${fieldHtmlId}-entry', '${fieldHtmlId}', <#if field.mandatory>true<#else>false</#if>);" <#if field.disabled && !(field.control.params.forceEditable?? && field.control.params.forceEditable == "true")>disabled="true"</#if> tabindex="0" type="checkbox" id="${fieldHtmlId}-${nameValue_index}" name="${fieldHtmlId}-entry"  value="<#if field.control.params.isSearch?? >=</#if><#if field.control.params.isSearch??>${nameValue?string?replace(" ","\\ ")?html}<#else>${nameValue?html}</#if>" <#if isSelected(nameValue) && !field.control.params.isSearch??>checked</#if>></input>
                    <label for="${fieldHtmlId}-${nameValue_index}" >${nameValue?html}</label></p>
                  <#else>
                     <#assign choice=nameValue?split(labelSeparator)>
                     <p>  <input   onchange="javascript:beCPG.util.updateMultiCheckboxesValue('${fieldHtmlId}-entry', '${fieldHtmlId}', <#if field.mandatory>true<#else>false</#if>);" <#if field.disabled && !(field.control.params.forceEditable?? && field.control.params.forceEditable == "true")>disabled="true"</#if> tabindex="0" type="checkbox" id="${fieldHtmlId}-${nameValue_index}" name="${fieldHtmlId}-entry"  value="<#if field.control.params.isSearch?? >=</#if><#if field.control.params.isSearch??>${choice[0]?string?replace(" ","\\ ")?html}<#else>${choice[0]?html}</#if>" <#if isSelected(choice[0]) && !field.control.params.isSearch??>checked</#if>></input>
                     <label for="${fieldHtmlId}-${nameValue_index}" >${msgValue(choice[1])?html}</label></p>
                    </#if>
                   </#if> 
               </#list>
