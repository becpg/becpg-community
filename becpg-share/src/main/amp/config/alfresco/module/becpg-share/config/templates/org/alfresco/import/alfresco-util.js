...
var AlfrescoUtil =
{
   ...
   getPaths: function getPaths(itemDetails, targetPage, targetPageLabel)
   {
      var item = itemDetails.item,
         ...
         pathUrl = "",
         x, y,
         isEntity = item.node.aspects.indexOf("bcpg:entityListsAspect") > 0;
         ...
         if (path.length > 1)
         {
            ...
            for (x = 0, y = folders.length; x < y; x++)
            {
               ...
               paths.push(
               {
                  ...
                  label: folders[x],
                  cssClass: "folder-link " + (y - x == 1 ?   (isEntity?  "folder-close entity "+ item.node.type.replace(":","-") : "folder-closed") : "folder-open")
               });
            }
         }
      }
