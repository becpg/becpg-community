...
/**
 * CreateTopic component. Logic for a topic creation form.
 * 
 * @namespace Alfresco
...
(function() {
	...
	Alfresco.CreateTopic = function(htmlId) {
		...
		/* Load YUI Components */
		Alfresco.util.YUILoaderHelper.require([ "datasource", "json",
				"connection", "event", "button", "menu", "editor" ],
	...
	Alfresco.CreateTopic.prototype = {
		...
		/**
		 * Called when the people finder template has been loaded. Creates a
		 * dialog and inserts the people finder for choosing supplier.
		 * 
		...
		onPeopleFinderLoaded : function UP_onPeopleFinderLoaded(response) {
			...
			this.widgets.reassignPanel = Alfresco.util.createYUIPanel(this.id
					+ "-reassignPanel");

			// Find the People Finder by container ID
			this.widgets.peopleFinder = Alfresco.util.ComponentManager
					.get(this.id + "-peopleFinder");

			// Set the correct options for our use
			this.widgets.peopleFinder.setOptions({
				singleSelectMode : true,
				addButtonLabel : this._msg("button.select")
			});
		...
		/**
		 * Called when the user has selected a supplier from the people finder.
		 * 
		...
		onPersonSelected : function UP_onPersonSelected(e, args) {
			// This is a "global" event so we ensure the event is for the
			// current panel by checking panel visibility.
			if ($hasEventInterest(this.widgets.peopleFinder, args)) {
				this.widgets.reassignPanel.hide();
				this.widgets.supplierUserName = args[1].userName;
				this._updatePeopleProperties({
					"supplier_substitute" : args[1].firstName +" "+
											args[1].lastName  +" ("+
											args[1].userName +")"
				}, "invite");
			}
		...
		/**
		 * Updates a people property supplier
		 * 
		 * @method: _updatePeopleProperties
		 * @private
		 */
		_updatePeopleProperties : function UP_updatepeopleProperties(
				properties, action) {
			Dom.get(this.id + "-supplier").value = properties.supplier_substitute;
		},
		...
		_loadDiscussionsTopicData : function CreateTopic__loadDiscussionsTopicData() {
			...
			var me = this;
			var loadTopicPostDataSuccess = function CreateTopic_loadTopicDataSuccess(
					response) {
				// set the topic data
				var data = response.json.item;
				me.discussionsTopicData = data;

				// now initialize the form, which will use the data we just
				// loaded
			...
			var url = YAHOO.lang
					.substitute(
							Alfresco.constants.URL_SERVICECONTEXT
									+ "components/forum/post/site/{site}/{container}/{topicId}",
							{
		...
		/**
		 * Initializes the create topic form dom. The html is already in the dom
		 * when the component gets loaded
		 */
		_initializeDiscussionsTopicForm : function CreateTopic_initializeDiscussionsTopicForm() {

			var actionUrl, draft = true, title = "", content = "", supplierAccountRef = null, nodeRef = "";

			if (this.options.editMode) {
				actionUrl = YAHOO.lang.substitute(Alfresco.constants.PROXY_URI
						+ "api/forum/post/site/{site}/{container}/{topicId}", {
					site : this.options.siteId,
					container : this.options.containerId,
					topicId : this.options.topicId
				});
			...
			Dom.get(this.id + "-site").setAttribute("value",
					this.options.siteId);
			Dom.get(this.id + "-container").setAttribute("value",
			...
			if (this.options.editMode) {
				...
				nodeRef = this.discussionsTopicData.nodeRef;
			}
			// supplierAccountRef
			if (this.options.editMode) {
				supplierAccountRef = this.discussionsTopicData.supplierAccountRef;
			}
			
			Dom.get(this.id + '-content').value = content;

			if(this.options.editMode && supplierAccountRef != null ){
				this.widgets.supplierUserName = supplierAccountRef.userName;
				this._updatePeopleProperties({
					"supplier_substitute" : supplierAccountRef.firstName +" "+
											supplierAccountRef.lastName +" ("+
											supplierAccountRef.userName +")"
				}, "invite");
			}
		...
		_registerCreateTopicForm : function CreateTopic__registerCreateTopicForm() {
			...
			if (this.options.editMode
					&& this.discussionsTopicData.tags.length > 0) {
				this.modules.tagLibrary.setTags(this.discussionsTopicData.tags);
			}

			// —
			this.widgets.reassignButton = Alfresco.util.createYUIButton(this,
					"reassign", this.onReassignButtonClick);

			// register the okButton
			this.widgets.okButton = new YAHOO.widget.Button(
					this.id + "-submit", {
						type : "submit"
					});
			YUIDom.addClass(
					this.widgets.okButton._button.parentElement.parentElement,
					"alf-primary-button");

			// register the cancel button
			this.widgets.cancelButton = new YAHOO.widget.Button(this.id
					+ "-cancel");
			this.widgets.cancelButton.subscribe("click",
					this.onFormCancelButtonClick, this, true);

			// instantiate the simple editor we use for the form
			this.widgets.editor = new Alfresco.util.RichEditor(
					Alfresco.constants.HTML_EDITOR, this.id + '-content',
					this.options.editorConfig);
			this.widgets.editor.addPageUnloadBehaviour(this
					._msg("message.unsavedChanges.discussion"));
			this.widgets.editor.render();

			// create the form that does the validation/submit
			this.widgets.topicForm = new Alfresco.forms.Form(this.id + "-form");
			// Title
			this.widgets.topicForm.addValidation(this.id + "-title",
					Alfresco.forms.validation.mandatory, null, "keyup");
			this.widgets.topicForm.addValidation(this.id + "-title",
					Alfresco.forms.validation.length, {
						max : 256,
						crop : true
					}, "keyup");
			// Content
			...
			if (this.options.editMode) {
				this.widgets.topicForm
						.setAjaxSubmitMethod(Alfresco.util.Ajax.PUT);
			}
			...
			this.widgets.topicForm.doBeforeFormSubmit = {
				fn : function(form, obj) {
					...
					// Put Supplier userName in the form
					if(this.widgets.supplierUserName){
						Dom.get(this.id + '-supplier').value = this.widgets.supplierUserName;
					}else {
						Dom.get(this.id + '-supplier').setAttribute("name", ""); 
					}
					
					
					// update the tags set in the form
					this.modules.tagLibrary.updateForm(this.id + '-form',
							'tags');

					// show a wait message
					this.widgets.feedbackMessage = Alfresco.util.PopupManager
							.displayMessage({
								text : Alfresco.util.message(this
										._msg("message.submitting")),
								spanClass : "wait",
								displayTime : 0
							});
				},
		 ...
		 */
		onFormSubmitSuccess : function CreateTopic_onFormSubmitSuccess(
				response, object) {		

			// the response contains the data of the created topic. redirect to
			// the topic view page
			var url = Alfresco.util.discussions.getTopicViewPage(
		 ...
		 */
		onFormCancelButtonClick : function CreateTopic_onFormCancelButtonClick(
				type, args) {
		...
		_msg : function CreateTopic_msg(messageId) {
			return Alfresco.util.message.call(this, messageId,
					"Alfresco.CreateTopic", Array.prototype.slice.call(
							arguments).slice(1));
		}
	};
})();