								...
								if (e.dataTransfer.files !== undefined && e.dataTransfer.files !== null && e.dataTransfer.files.length > 0) {
									...
									if (continueWithUpload) {
										...
										if (items[0] && items[0].webkitGetAsEntry && (firstEntry = items[0].webkitGetAsEntry())) {
											// detected uploading of folders
											// structure (only supported by
											// Chrome >= 21)
											fnWalkFileSystem(firstEntry.filesystem.root, function(fs, dirs) {
												...
												// collapse folder list by
												// removing folders with empty
												// parents - we want empty leaf
												// nodes only
