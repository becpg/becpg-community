    ...
    YAHOO
        .extend(
            ...
            Alfresco.component.Base,
            {
                ...
                onReady: function AutoCompletePicker_onReady() {
                    ...
                    if (me.options.mode != "view" && !this.options.readOnly) {
                        ...
                        me.widgets.oAC.textboxKeyEvent.subscribe(function() {
                            me.widgets.oAC.page = 1;
                            previewTooltips = [];
                            // FIX #33135-regression : toute frappe clavier est une action
                            // utilisateur → marquer le champ DOM pour autoriser checkParentField
                            // des enfants. Si le champ est vidé (Suppr), on fire l'événement
                            // immédiatement pour que l'enfant se vide sans attendre le blur.
                            var inputEl = me.widgets.oAC.getInputEl();
                            if (inputEl) {
                                inputEl._userModified = true;
                                ...
                                if (me.openOnce && (!me.widgets.oAC._bOverContainer || (me.widgets.oAC._nKeyCode == 9))) {
                                    ...
                                    if (!me.widgets.oAC._bItemSelected) {
                                        ...
                                        if (!me.widgets.oAC._bContainerOpen || (me.widgets.oAC._bContainerOpen && (elMatchListItem === null))) {
                                            ...
                                            // current query
                                            me.widgets.oAC._clearSelection();
                                            // FIX #33135-regression : l'utilisateur a effacé
                                            // le parent manuellement → marquer le DOM element
                                            me.widgets.oAC.getInputEl()._userModified = true;
                                            YAHOO.Bubbling.fire("mandatoryControlValueUpdated", me.widgets.oAC.getInputEl());
                                        }
                                    }
                                }
                        ...
                        me.widgets.oAC.itemSelectEvent.subscribe(function(type, args) {
                            ...
                            // FIX #33135-regression : marquer l'élément DOM du champ courant
                            // comme modifié par l'utilisateur, pour que checkParentField des
                            // enfants puisse détecter une vraie action utilisateur.
                            var inputEl = me.widgets.oAC.getInputEl();
                            if (inputEl) { inputEl._userModified = true; }
                            ...
                            // FIX #33240 / #33135 : Pour les propriétés (non-assoc, single select),
                            // le hidden "-added" n'était jamais renseigné → valeur absente au POST.
                            if (!me.options.multipleSelectMode && !me.isAssoc) {
                                var inputAddedProp = Dom.get(me.controlId + "-added");
                                if (inputAddedProp) {
                                    inputAddedProp.value = itemValue;
                                }
                            }
                ...
                // FIX #33240 / #33135 / #33135-regression :
                //
                // Problème 1 — garde self :
                //   Si parentFieldHtmlId == fieldHtmlId, le champ se vidait lui-même → on sort.
                //
                // Problème 2 — vidage des hidden fields :
                //   _clearSelection() effaçait l'affichage mais pas les hidden fields
                //   → la valeur obsolète partait quand même au POST.
                //
                // Problème 3 — régression au chargement (cas productHierarchy1/2) :
                //   renderItems() fire mandatoryControlValueUpdated au boot, ce qui
                //   déclenchait checkParentField et vidait la Sous-famille dès l'ouverture.
                //   Correctif : on n'agit que si le parent a été modifié par l'utilisateur,
                //   ce qu'on détecte via le flag parentUserModified positionné par
                //   itemSelectEvent et textboxBlurEvent (actions utilisateur réelles uniquement).
                checkParentField: function(layer, args) {
                    // Garde 1 : ne jamais se vider soi-même
                    if (this.options.parentFieldHtmlId === this.fieldHtmlId) {
                        return;
                    }

                    if (this.options.parentFieldHtmlId != null && args != null && args[1].id == this.options.parentFieldHtmlId) {

                        // Garde 2 : n'agir que si c'est une vraie modification utilisateur,
                        // pas le fire automatique émis par renderItems() au chargement.
                        // Le flag _userModified est posé sur l'élément DOM du parent par
                        // itemSelectEvent et textboxBlurEvent de l'instance parente.
                        var parentInputEl = Dom.get(this.options.parentFieldHtmlId);
                        if (!parentInputEl || !parentInputEl._userModified) {
                            return;
                        }
                        ...
                        // Vider aussi les hidden fields pour que le POST ne renvoie pas la valeur obsolète
                        ...
                        if (this.options.multipleSelectMode) {
                            ...
                            if (this.isAssoc && inputOrig && inputOrig.value !== "" && inputRemoved) {
                                inputRemoved.value = inputOrig.value;
                            }
                            if (inputAdded) { inputAdded.value = ""; }
                        }
