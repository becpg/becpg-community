   ...
   */
   ...
   YAHOO.extend(Alfresco.module.DoclibAspects, Alfresco.module.SimpleDialog,
   {
      ...
      _createAspectsControls: function DA__createAspectsControls()
      {
         ...
         var sortAspects = function(a, b, desc) {
             ...
             return comp(a.getData("name"), b.getData("name"), desc);
         };
         ...
         [
            ...
            { key: "name", label: "name", sortable: true, formatter: renderCellName, sortOptions:{sortFunction:sortAspects}, defaultDir: "asc"},
         ...
         [
            { key: "icon", label: "icon", sortable: false, formatter: renderCellIcon, width: 10 },
            { key: "name", label: "name", sortable: true, formatter: renderCellName },
      ...
      _requestAspectDataSuccess: function DA__requestAspectDataSuccess(response)
      {
         ...
         if (typeof response.json != "undefined")
         {
            ...
            var current, addable, record, rightTableRecords = [], leftTableRecords = [];
            ...
            for (i = 0, ii = currentArr.length; i < ii; i++)
            {
               ...
               {
            	   rightTableRecords.push(record);
               }
               this.selectedValues[current] = record;
            }
            
            rightTableRecords.sort(function(a, b) {
                return YAHOO.util.Sort.compare(a.name , b.name);
            });
            
            for(var i in rightTableRecords){
            	this.widgets.dataTableRight.addRow(rightTableRecords[i]);
            }
            ...
            for (i = 0, ii = addableArr.length; i < ii; i++)
            {
               ...
               if ((addable in visibleObj) && !(addable in currentObj) && this.i18n(addable).indexOf("undefined")<0)
               {
                 leftTableRecords.push(
                  {
                     id: addable,
                     icon: Alfresco.constants.URL_RESCONTEXT + "components/images/aspect-16.png",
            ...
            leftTableRecords.sort(function(a, b) {
                return YAHOO.util.Sort.compare(a.name , b.name);
            });
            
            for(var i in leftTableRecords){
            	this.widgets.dataTableLeft.addRow(leftTableRecords[i]);
            }
