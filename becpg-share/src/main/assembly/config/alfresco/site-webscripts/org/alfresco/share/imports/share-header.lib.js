   ...
   if (page.url.templateArgs.site)
   {
      ...
      if (siteData != null)
      {
         ...
         if (page.id != "documentlibrary" && page.id != "document-details" && page.id != "folder-details" && !page.url.uri.endsWith("/faceted-search"))
         {
            ...
            {
               if (!user.isAdmin && siteData.profile.visibility != "PUBLIC" 
               && siteData.profile.visibility != "MODERATED" && siteData.userIsMember === false
               && !(page.url.url.includes("/discussions-topicview") || page.url.uri.includes("/discussions-topicview"))) // beCPG
               {
                  services.push({
                     name: "share/services/UrlUnavailableService",
                     config: {
