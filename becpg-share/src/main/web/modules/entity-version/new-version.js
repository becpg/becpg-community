 ...
 * 
 * Popups a YUI panel and lets the user choose version and comment for the new
 * version.
 * 
...
(function()
{
     ...
     * 
     * NewEntityVersion is considered a singleton so constructor should be
     * treated as private, please use
     * Alfresco.module.getNewEntityVersionInstance() instead.
     * 
     * @param {string}
     *            htmlId The HTML id of the parent element
     * @return {Alfresco.module.NewEntityVersion} The new NewEntityVersion
     *         instance
     * @constructor
     * @private
     */
    Alfresco.module.NewEntityVersion = function(containerId)
    {
        ...
        // Load YUI Components
        Alfresco.util.YUILoaderHelper.require([ "button", "container", "datatable", "datasource" ],
    ...
    Alfresco.module.NewEntityVersion.prototype =
    {
        ...
        /**
         * The default config for the gui state for the newVersion dialog. The
         * user can override these properties in the show() method.
         * 
        ...
        /**
         * Show can be called multiple times and will display the newVersion
         * dialog in different ways depending on the config parameter.
         * 
         * @method show
         * @param config
         *            {object} describes how the dialog should be displayed The
         *            config object is in the form of: { nodeRef: {string}, //
         *            the nodeRef to version version: {string} // the version }
         */
        show : function RV_show(config)
        {
            ...
            if (this.showConfig.nodeRef === undefined && this.showConfig.filename === undefined && this.showConfig.version === undefined)
            {
            ...
            Alfresco.util.Ajax
                    .request(
                    {
                        url : Alfresco.constants.URL_SERVICECONTEXT + "modules/entity-version/new-version?htmlid=" + this.id + "&merge=" + this.showConfig.merge,
                        successCallback :
                        {
        ...
        /**
         * Called when the new version dialog html template has been returned
         * from the server. Creates the YIU gui objects such as the panel.
         * 
        ...
        onTemplateLoaded : function RV_onTemplateLoaded(response)
        {
            ...
            if (this.showConfig.merge)
            {
                this.widgets.autocomplete = new beCPG.component.AutoCompletePicker(this.id + '-entities',
                        this.id + '-entities-field', true).setOptions(
                {
                    mode : "edit",
                    multipleSelectMode : false,
                    isMandatory : true,
                    dsStr : "becpg/autocomplete/branches?entityNodeRef=" + this.showConfig.nodeRef
                });
            }
            ...
            // The ok button is the submit button, and it should be enabled when
            // the form is ready
        ...
        onNewVersionSuccess : function RV_onNewVersionSuccess(response)
        {
            ...
            {
                // Call the onNewEntityVersionComplete callback in the correct
                // scope
                callback.fn
                        .call((typeof callback.scope == "object" ? callback.scope : this), objComplete, callback.obj);
            }
        },

        /**
         * Called when a node failed to be versioned Informs the user.
         * 
        ...
        _applyConfig : function RV__applyConfig()
        {
            ...
            if (!this.showConfig.merge)
            {
                ...
                var versions = (this.showConfig.version || "1.0").split("."), majorVersion = parseInt(versions[0], 10), minorVersion = parseInt(
                        versions[1], 10);
                Dom.get(this.id + "-minorVersion").innerHTML = Alfresco.util.message("label.minorVersion.more",
                        this.name,
                        {
                            "0" : majorVersion + "." + (1 + minorVersion)
                        });
                Dom.get(this.id + "-majorVersion").innerHTML = Alfresco.util.message("label.majorVersion.more",
                        this.name,
                        {
                            "0" : (1 + majorVersion) + ".0"
                        });
            ...
            {
                var header = Alfresco.util.message("header.merge-branch", this.name,
                {
                    "0" : this.showConfig.filename
                });
                this.widgets.headerText["innerHTML"] = header;
