...
(function() {
   ...
   beCPG.module.EntityDataGrid = function(htmlId, scopeable) {
      ...
      beCPG.module.EntityDataGrid.superclass.constructor.call(this, "beCPG.module.EntityDataGrid", htmlId, [ "button",
            "container", "datasource", "datatable", "paginator", "animation", "history" ]);
         ...
         .augmentObject(
               beCPG.module.EntityDataGrid.prototype,
               {
                  ...
                  options : {
                     ...
                     /**
                      * Flag indicating whether filter is available or not.
                      * 
                      * @property useFilter
                      * @type boolean
                      * @default false
                     ...
                     /**
                      * Item type : Force item Type let it null for default
                      */
                     itemType : null,
                  ...
                  /**
                   * Deferred function calls for after a data grid update and not deleted after
                   * 
                  ...
                  onReady : function EntityDataGrid_onReady() {
                     ...
                     // Reference to Data Grid component (required by actions
                     // module)
                     ...
                     if (this.options.useFilter) {
                        ...
                        this.widgets.filterFormSubmit = Alfresco.util.createYUIButton(this, "filterform-submit",
                        ...
                        this.widgets.filterForm = Alfresco.util.createYUIButton(this, "filterform-button", null, {
                           type : "menu",
                  ...
                  /**
                   * Fired by YUI when History Manager is initialised and available for scripting. Component
                   * initialisation, including instantiation of YUI widgets and event listener binding.
                   * 
                   ...
                   * @param response
                   *            {Object} Server response object from Ajax request wrapper
                   * @param message
                  ...
                  _getColumnUrl : function (formId){
                     return this.options.columnsUrl + "?itemType=" + encodeURIComponent(this.options.itemType != null ? this.options.itemType
                           : this.datalistMeta.itemType) + "&list=" + encodeURIComponent(this.options.list)+(formId!=null?"&formId="+formId:"");
                  },
                   ...
                   * @param response
                   *            {object} Server response from load template XHR request
                   */
                  ...
                  _setupHistoryManagers : function EntityDataGrid__setupHistoryManagers() {
                     ...
                     bookmarkedFilter = bookmarkedFilter === null ? "all" : (YAHOO.env.ua.gecko > 0) ? bookmarkedFilter
                           : window.escape(bookmarkedFilter);
                     ...
                     if (this.options.initHistoryManager) {
                        try {
                           ...
                           /*
                            * The only exception that gets thrown here is when the browser is not supported (Opera, or
                            * not A-grade)
                            */
                        ...
                        Bubbling.fire("dataGridReady", {entityDataGridModule : this});
                  ...
                  _setupDataSource : function EntityDataGrid__setupDataSource() {
                    var me = this;
                     ...
                     // Intercept data returned from data webscript to extract
                     // custom metadata
                     this.widgets.dataSource.doBeforeCallback = function EntityDataGrid_doBeforeCallback(oRequest,
                                                                                                         oFullResponse,
                                                                                                         oParsedResponse) {
                  ...
                  _getDataUrl : function (pageSize){
                     var listNodeRef = this.datalistMeta.nodeRef != null ? new Alfresco.util.NodeRef(
                       ...
                       return this.options.dataUrl + (listNodeRef != null ? listNodeRef.uri : "") + ((this.options.entityNodeRef != null && this.options.entityNodeRef.length > 0) ? "?entityNodeRef=" + this.options.entityNodeRef + "&"
                                 ...
                                 : this.options.list) + "&pageSize=" + (pageSize!=null?pageSize : this.options.pageSize) + "&site=" + this.options.siteId + this
                                 ._buildSortParam() + this.options.extraDataParams;
                  },
                  /**
                   * DataTable set-up and event registration
                   * 
                  ...
                  _setupDataTable : function EntityDataGrid__setupDataTable(columns) {
                     ...
                     for ( var i = 0, ii = this.datalistColumns.length; i < ii; i++) {
                        ...
                        if (this.options.hiddenColumns.length < 1 || !beCPG.util.contains(this.options.hiddenColumns,
                              key)) {
                           columnDefinitions.push({
                              ...
                              editor : this.options.saveFieldUrl != null ? this.rendererHelper.getCellEditor(this,
                                    column, this.options.saveFieldUrl) : null
                           });
                        }
                     }
                     ...
                     this.widgets.dataTable = new YAHOO.widget.GroupedDataTable(this.id + "-grid", columnDefinitions,
                           this.widgets.dataSource, dataTableOptions);
                     ...
                     if (this.options.usePagination) {
                        ...
                        // Add the containers if the Paginator is not configured
                        // with containers
                     ...
                     this.widgets.dataTable.doBeforeLoadData = function EntityDataGrid_doBeforeLoadData(sRequest,
                                                                                                        oResponse,
                                                                                                        oPayload) {
                        ...
                        // We don't get an renderEvent for an empty recordSet,
                     ...
                     // Override default function so the "Loading..." message is
                     // suppressed
                     ...
                     this.widgets.dataTable
                           .subscribe(
                                 ...
                                 function() {
                                    if (me.currentSort) {
                                       // Is there a custom sort handler
                                       // function defined?
                                       ...
                                       if (sSortDir || sortFnc) {
                                          ...
                                          // Sort the Records
                                          this._oRecordSet.sortRecords(sortFnc,
                                                ((sSortDir == YAHOO.widget.DataTable.CLASS_DESC) ? true : false),
                                                sField);
                                       }
                                    }
                     ...
                     if (this.options.saveFieldUrl != null) {
                        ...
                        this.widgets.dataTable.subscribe("tbodyKeyEvent",    function(oArgs){
                           ...
                           if((nKey > 47  && nKey < 90) ||(nKey > 95  && nKey < 106) ){
                              showCellEditor({target : this.getLastSelectedCell()});
                        ...
                        this.widgets.dataTable.subscribe("cellSelectEvent", this.widgets.dataTable.clearTextSelection);
                        ...
                        var onRowSelect = function(ev) {
                           if (me.widgets.dataTable.getRecord(Event.getTarget(ev)) != null) {
                              ...
                              if (userAccess.sort) {
                                 ...
                                 /**
                                  * Once we start dragging a row, we make the proxyEl look like the src Element. We get
                                  * also cache all the data related to the row
                                  * 
                                  * @return void
                                  * @static
                                  * @method startDrag
                                    ...
                                    if (me.options.sortUrl) {
                                       ...
                                       if (destRecord != null) {
                                          ...
                                          if (dstData && srcData) {
                                             ...
                                             var url = me.options.sortUrl + "/" + dstData.nodeRef.replace(":/", "") + "?selectedNodeRefs=" + srcData.nodeRef;
                  ...
                  onEventHighlightRow : function EntityDataGrid_onEventHighlightRow(oArgs) {
                     ...
                     // elActions is the element id of the active table cell
                     // where we'll inject the actions
                     var elActions = Dom.get(this.id + "-actions-" + oArgs.target.id);

                     // Inject the correct action elements into the actionsId
                     // element
                     if (elActions && elActions.firstChild === null) {
                        ...
                        // Trim the items in the clone depending on the user's
                        // access
                        var userAccess = record.getData("permissions").userAccess, actionLabels = record
                              .getData("actionLabels") || {};

                        // Inject the current filterId to allow filter-scoped
                        // actions
                        userAccess["filter-" + this.currentFilter.filterId] = true;

                        // Remove any actions the user doesn't have permission
                        // for
                        ...
                        if (actions.length > splitAt) {
                           ...
                           // Insert the two necessary DIVs before the splitAt
                           // action item
                           Dom.insertBefore(containerDivs[0], actions[splitAt]);
                           Dom.insertBefore(containerDivs[1], actions[splitAt]);
                           // Now make action items after the split, children of
                           // the 2nd DIV
                  ...
                  onEventUnhighlightRow : function EntityDataGrid_onEventUnhighlightRow(oArgs) {
                     ...
                     // Don't hide unless the More Actions drop-down is showing,
                     // or a dialog mask is present
                     if (!this.showingMoreActions || Dom.hasClass(document.body, "masked")) {
                        // Just hide the action links, rather than removing them
                        // from the DOM
                   ...
                   * @param record
                   *            {YAHOO.widget.Record | Object} A data record, or object literal describing the item in
                   *            the list
                  ...
                  /**
                   * Public functions Functions designed to be called form external sources
                   */
                  ...
                  /**
                   * Selected Items Changed event handler. Determines whether to enable or disable the multi-item action
                   * drop-down
                   * 
                  ...
                  onSelectedItemsChanged : function DataListToolbar_onSelectedItemsChanged(layer, args) {
                     ...
                     for (i = 0, ii = items.length; i < ii; i++) {
                        ...
                        // Required user access level - logical AND of each
                        // item's permissions
                     ...
                     // Now go through the menu items, setting the disabled flag
                     // appropriately
                     for ( var index in menuItems) {
                        if (menuItems.hasOwnProperty(index)) {
                           ...
                           if (menuItem.element.firstChild) {
                              // Check permissions required - stored in "rel"
                              ...
                              if (menuItem.element.firstChild.rel && menuItem.element.firstChild.rel !== "") {
                                 ...
                                 for (i = 0, ii = actionPermissions.length; i < ii; i++) {
                                    // Disable if the user doesn't have ALL the
                                    // permissions
                     ...
                     if (obj && obj.userAccess) {
                        ...
                        for ( var index in this.widgets) {
                           if (this.widgets.hasOwnProperty(index)) {
                              ...
                              if (widget["get"] && widget.get("srcelement") != null && widget.get("srcelement").className != "no-access-check") {
                                 // Default to disabled: must be enabled via
                                 // permission
                                 ...
                                 if (typeof widget.get("value") == "string") {
                                    ...
                                    for ( var i = 0, ii = widgetPermissions.length; i < ii; i++) {
                                       // Pipe-separation is a special case and
                                       // indicates an "OR" match. The matched
                                       // permission is stored in
                                       // "activePermission" on the widget.
                   ...
                   * @param record
                   *            {object} Object literal representing file or folder to be actioned
                   * @param elMore
                   *            {element} DOM Element of "More Actions" link
                   */
                                    ...
                                    fn : function EntityDataGrid_onDataItemCreated_refreshSuccess(response) {
                                       ...
                                       if (response.json && (response.json.item !== null)) {
                                          ...
                                          if (response.json.lastSiblingNodeRef != null) {
                                             ...
                                             if (prevRecord !== null) {
                                                var idx = this.widgets.dataTable.getRecordIndex(prevRecord);
                  ...
                  onColumnRenamed : function EntityDataGrid_onColumnRenamed(layer, args) {
                     ...
                     if (obj && (obj.columnId !== null)) {
                        ...
                        if (oColumn) {
                           ...
                           this.widgets.dataTable.formatTheadCell(oColumn._elThLabel, oColumn, this.widgets.dataTable
                                 .get("sortedBy"));
                   ...
                   * @private
                   * @param p_value
                   *            {string} Value to find (to get the previous)
                   * @param p_parameter
                   *            {string} Parameter to look for the value in
                   */
                  ...
                  /**
                   * Searches the recordSet for the prev item of a record with the given parameter value
                   * 
                   ...
                   *            {string} Parameter to look for the value in
                   */
                  _findPrevItemByParameter : function EntityDataGrid__findNextItemByParameter(p_value, p_parameter) {
                     var recordSet = this.widgets.dataTable.getRecordSet();
                     for ( var i = 0, j = recordSet.getLength(); i < j; i++) {
                        if (recordSet.getRecord(i).getData(p_parameter) == p_value) {
                           if ((i - 1) >= 0) {
                  ...
                  /**
                   * Resets the YUI DataTable errors to our custom messages NOTE: Scope could be YAHOO.widget.DataTable,
                   * so can't use "this"
                   * 
                  ...
                  _updateDataGrid : function EntityDataGrid__updateDataGrid(p_obj) {
                     ...
                     // Clear the current document list if the data webscript is
                     // taking too long
                     var fnShowLoadingMessage = function EntityDataGrid_fnShowLoadingMessage() {
                        // Check the timer still exists. This is to prevent IE
                        // firing the event after we cancelled it. Which is
                        // "useful".
                     ...
                     destroyLoaderMessage = function EntityDataGrid__uDG_destroyLoaderMessage() {
                        ...
                        if (loadingMessage) {
                           if (this.loadingMessageShowing) {
                              ...
                              // Wait and try again later. Scope doesn't get set
                              // correctly with "this"
                     ...
                     var successHandler = function EntityDataGrid__uDG_successHandler(sRequest, oResponse, oPayload) {
                        ...
                        if(p_obj.updateOnly && this.scopeId == ""){
                           this.widgets.dataTable.onDataReturnUpdateRows.call(this.widgets.dataTable, sRequest,
                                 oResponse, oPayload);
                           if(p_obj.callback){
                          ...
                          // Updating the DataGrid may change the item selection 
                           ...
                           this.widgets.dataTable.onDataReturnReplaceRows.call(this.widgets.dataTable, sRequest,
                                 oResponse, oPayload);
                        }
                     };
                        ...
                        if (oResponse.status == 401) {
                           ...
                           try {
                              ...
                              if (oResponse.status == 404) {
                                 // Site or container not found - deactivate
                                 // controls
                     ...
                     if(me.options.postMethod){
                        ...
                        requestParams = "&metadata="+encodeURIComponent(YAHOO.lang.JSON.stringify(requestParams));
                     }
                   ...
                   * @return {Object} Request parameters. Can be given directly to Alfresco.util.Ajax, but must be
                  ...
                  _buildDataGridParams : function EntityDataGrid__buildDataGridParams(p_obj) {
                     ...
                     if (p_obj && p_obj.filter) {
                        request.filter = {
                           ...
                           filterParams : this._createFilterURLParameters(p_obj.filter, this.options.filterParameters)
                        };
                     }
                   ...
                   *            {Array} List of configured filter parameters that shall create url parameters
                   * @return URL parameters created from the instructions in filterParameters based on data from the
                   *         filter OR null no instructions were found
                   * @override
                   */
                  _createFilterURLParameters : function DateFilter_createFilterURLParameters(filter, filterParameters) {
                  ...
                  /**
                   * Takes a template and performs substituion against "Obj" and according to date instructions as
                   * described below. Assumes the template data may contain date instructions where the instructions are
                   * placed inside curly brackets: "param={attr}" - the name of an attribute in "obj" "param={0dt}" -
                   * the current date time in iso8601 format "param={1d}" - the current date (time set to end of day)
                   * and rolled l days forward "param={-2d}" - the current date (time set to end of day) and rolled 2
                   * days backward
                   * 
                   * @param template
