...
(function() {
	...
	beCPG.module.EntityDataGrid = function(htmlId, scopeable) {
		...
		beCPG.module.EntityDataGrid.superclass.constructor.call(this, "beCPG.module.EntityDataGrid", htmlId, [ "button", "container", "datasource",
				"datatable", "paginator", "animation", "history" ]);
			...
			.augmentObject(
					beCPG.module.EntityDataGrid.prototype,
					{
						...
						_buildCellUrl : function EntityDataGridRenderers__buildCellUrl(data) {
							...
							if (data.siteId) {
								url = Alfresco.constants.URL_PAGECONTEXT + "site/" + data.siteId + "/" + containerType + '-details?nodeRef='
										+ data.value;
							} else {
						...
						onReady : function EntityDataGrid_onReady() {
							...
							// new row button
							this.widgets.newRowButton = Alfresco.util.createYUIButton(this, "newRowButton", this.onActionCreate, {
								disabled : false,
								value : "create"
							});
							...
							if (this.options.useFilter) {
								...
								this.widgets.filterFormSubmit = Alfresco.util.createYUIButton(this, "filterform-clear", this.onFilterFormClear);
						...
						renderDataListMeta : function EntityDataGrid_renderDataListMeta() {
							...
							Alfresco.util.populateHTML([ this.id + "-title", $html(this.datalistMeta.entityName + " - " + this.datalistMeta.title) ],
							...
							var filterFormUrl = YAHOO.lang
									.substitute(
											Alfresco.constants.URL_SERVICECONTEXT
													+ "module/entity-datagrid/filter/form?itemKind={itemKind}&list={list}&itemId={itemId}&formId={formId}&submitType=json&showCancelButton=false&showSubmitButton=false"
													+ ((this.options.entityNodeRef != null && this.options.entityNodeRef.length > 0) ? "&entityNodeRef="
															+ this.options.entityNodeRef
															: ""), {
												itemKind : "type",
												itemId : this.options.itemType != null ? this.options.itemType : this.datalistMeta.itemType,
												formId : "filter",
												list : listName,
												submitType : "json"
											});
						...
						onFilterFormSubmit : function EntityDataGrid_onFilterFormSubmit() {
							Bubbling.fire(this.scopeId + "changeFilter", {
								filterOwner : this.id,
								filterId : "filterform",
								filterData : YAHOO.lang.JSON.stringify(this.cleanFilterData(this.formsFilterRuntime.getFormData())).replace("|",
										"$ML$")
							});
						...
						_setupHistoryManagers : function EntityDataGrid__setupHistoryManagers() {
							...
							bookmarkedFilter = bookmarkedFilter === null ? "all" : (YAHOO.env.ua.gecko > 0) ? bookmarkedFilter : window
									.escape(bookmarkedFilter);
						...
						_setupDataSource : function EntityDataGrid__setupDataSource() {
							...
							// extract
							// custom metadata
							this.widgets.dataSource.doBeforeCallback = function EntityDataGrid_doBeforeCallback(oRequest, oFullResponse,
									oParsedResponse) {
						...
						_getDataUrl : function(pageSize) {
							var listNodeRef = this.datalistMeta.nodeRef != null ? new Alfresco.util.NodeRef(this.datalistMeta.nodeRef) : null;

							return this.options.dataUrl
									+ (listNodeRef != null ? listNodeRef.uri : "")
									+ ((this.options.entityNodeRef != null && this.options.entityNodeRef.length > 0) ? "?entityNodeRef="
											+ this.options.entityNodeRef + "&" : "?") + "itemType="
									+ encodeURIComponent(this.options.itemType != null ? this.options.itemType : this.datalistMeta.itemType)
									+ "&dataListName="
									+ encodeURIComponent(this.datalistMeta.name != null ? this.datalistMeta.name : this.options.list) + "&pageSize="
									+ (pageSize != null ? pageSize : this.options.pageSize) + "&site=" + this.options.siteId + this._buildSortParam()
						...
						_setupDataTable : function EntityDataGrid__setupDataTable(columns) {
							...
							for (var i = 0, ii = this.datalistColumns.length; i < ii; i++) {
								...
								if (this.options.hiddenColumns.length < 1 || !beCPG.util.contains(this.options.hiddenColumns, key)) {
									columnDefinitions.push({
										...
										editor : this.options.saveFieldUrl != null ? this.rendererHelper.getCellEditor(this, column,
												this.options.saveFieldUrl) : null
									});
								}
							}
							...
							this.widgets.dataTable = new YAHOO.widget.GroupedDataTable(this.id + "-grid", columnDefinitions, this.widgets.dataSource,
									dataTableOptions);
							...
							if (this.options.usePagination) {
								...
								if (!this.widgets.paginator.getContainerNodes().length) {
									this.widgets.paginator.set('containers', this.widgets.dataTable._defaultPaginatorContainers(true));
								}
											...
											function() {
												if (me.currentSort) {
													...
													if (sSortDir || sortFnc) {
														...
														// Sort the Records
														this._oRecordSet.sortRecords(sortFnc, ((sSortDir == YAHOO.widget.DataTable.CLASS_DESC) ? true
																: false), sField);
													}
												}
							...
							if (this.options.sortable) {
								...
								var onRowSelect = function(ev) {
									if (me.widgets.dataTable.getRecord(Event.getTarget(ev)) != null) {
										var userAccess = me.widgets.dataTable.getRecord(Event.getTarget(ev)).getData("permissions").userAccess;
										if (userAccess.sort) {
												...
												if (me.options.sortUrl) {
													...
													if (destRecord != null) {
														...
														if (dstData && srcData) {
															...
															var url = me.options.sortUrl + "/" + dstData.nodeRef.replace(":/", "")
																	+ "?selectedNodeRefs=" + srcData.nodeRef;
						...
						onEventHighlightRow : function EntityDataGrid_onEventHighlightRow(oArgs) {
							...
							if (elActions && elActions.firstChild === null) {
								...
								// DOM
								var record = this.widgets.dataTable.getRecord(oArgs.target.id), clone = Dom.get(this.id + "-actionSet").cloneNode(
										true);
								...
								// user's
								// access
								var userAccess = record.getData("permissions").userAccess, actionLabels = record.getData("actionLabels") || {};
						...
						onSelectedItemsChanged : function DataListToolbar_onSelectedItemsChanged(layer, args) {
							var items = this.getSelectedItems(), item, userAccess = {}, itemAccess, menuItems = this.widgets.selectedItems.getMenu()
									.getItems(), menuItem, actionPermissions, disabled, i, ii;
							...
							if (obj && obj.userAccess) {
								...
								for ( var index in this.widgets) {
									if (this.widgets.hasOwnProperty(index)) {
										...
										// "no-access-check"
										if (widget["get"] && widget.get("srcelement") != null
												&& widget.get("srcelement").className != "no-access-check") {
						...
						onChangeFilter : function EntityDataGrid_onChangeFilter(layer, args) {
							...
							if ((obj !== null) && (obj.filterId !== null)) {
								...
								var filter = Alfresco.util.cleanBubblingObject(obj), strFilter = YAHOO.lang.substitute("{filterId}|{filterData}",
										filter, function(p_key, p_value, p_meta) {
						...
						onDataItemCreated : function EntityDataGrid_onDataItemCreated(layer, args) {
							...
							if (obj && (obj.nodeRef !== null)) {
								...
								var nodeRef = new Alfresco.util.NodeRef(obj.nodeRef), url = this.options.itemUrl
										...
										+ encodeURIComponent(this.options.itemType != null ? this.options.itemType : this.datalistMeta.itemType)
										+ "&dataListName="
										+ encodeURIComponent(this.datalistMeta.name != null ? this.datalistMeta.name : this.options.list) + "&site="
										+ this.options.siteId;
										...
										fn : function EntityDataGrid_onDataItemCreated_refreshSuccess(response) {
											...
											if (response.json && (response.json.item !== null)) {
												...
												if (response.json.lastSiblingNodeRef != null) {
													var prevRecord = this._findRecordByParameter(response.json.lastSiblingNodeRef, "nodeRef");
													if (prevRecord !== null) {
						...
						onColumnRenamed : function EntityDataGrid_onColumnRenamed(layer, args) {
							...
							if (obj && (obj.columnId !== null)) {
								...
								if (oColumn) {
									...
									this.widgets.dataTable.formatTheadCell(oColumn._elThLabel, oColumn, this.widgets.dataTable.get("sortedBy"));
						...
						_updateDataGrid : function EntityDataGrid__updateDataGrid(p_obj) {
							p_obj = p_obj || {};
							var successFilter = YAHOO.lang.merge({}, p_obj.filter !== undefined ? p_obj.filter : this.currentFilter), loadingMessage = null, timerShowLoadingMessage = null, me = this, params = {
							...
							var successHandler = function EntityDataGrid__uDG_successHandler(sRequest, oResponse, oPayload) {
								...
								if (p_obj.updateOnly && this.scopeId == "") {
									this.widgets.dataTable.onDataReturnUpdateRows.call(this.widgets.dataTable, sRequest, oResponse, oPayload);
									if (p_obj.callback) {
							...
							if (me.options.postMethod) {
								...
								if (Alfresco.util.CSRFPolicy.isFilterEnabled()) {
									this.widgets.dataSource.connMgr.initHeader(Alfresco.util.CSRFPolicy.getHeader(), Alfresco.util.CSRFPolicy
											.getToken(), false);
								}
