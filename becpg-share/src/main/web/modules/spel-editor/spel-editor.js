	      ...
	      .extend(
	            ...
	            Alfresco.component.Base,
	            {
	               ...
	               options : {
	                  ...
	                  /**
							 * The selected value to be displayed (but not yet
							 * persisted)
							 * 
	               ...
	               /**
						 * Selected items. Keeps a list of selected items for correct
						 * Add button state.
						 * 
	               ...
	               onParentTypeChanged : function SpelEditor_onParentTypeChanged(layer, args) {
		               ...
		               if ($hasEventInterest(this, args)) {
			               var obj = args[1];
			               if (obj && obj.item) {
				               this.widgets.itemTypeMenu.set("label", '<div><span class="item-icon"><img src="'
				                     + Alfresco.constants.URL_RESCONTEXT
				                     + 'components/form/images/ajax_anim.gif" width="16" height="16" alt="'
				                     + this.msg("message.please-wait") + '"></span><span class="item-name">'
				                     + this._formatLabel(obj.item.name) + '</span></div>');
	               ...
	               /**
						 * Gets selected or current value's metadata from the
						 * repository
						 * 
		               ...
		               if (this.options.disabled === false) {
			               ...
			               this.widgets.dataTable1.doBeforeLoadData = function(sRequest, oResponse, oPayload) {
				               if (me.options.selectedParentType) {
					               ...
					               label = "<span class='" + me.options.selectedParentType.name
					                     + "' style='padding-left: 20px;' >"
					                     + me._formatLabel(me.options.selectedParentType.name) + "</span>";
	               ...
	               /**
						 * Fires the refreshItemList event to refresh the contents of
						 * the editor.
						 * 
