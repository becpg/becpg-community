...
(function()
{
   ...
   {
      beCPG.SpelEditor.superclass.constructor.call(this, "beCPG.SpelEditor", htmlId, ["button", "menu", "container", "resize", "datasource", "datatable"]);
   ...
   YAHOO.extend(beCPG.SpelEditor, Alfresco.component.Base,
   {
      /**
      ...
      options:
      {
          ...
          * Datasource
          */
         dsr : Alfresco.constants.PROXY_URI +"becpg/autocomplete/speleditor" //${field.dataType};
      ...
      onReady: function SpelEditor_onReady()
      {
         ...
         if (!this.options.disabled)
         {
            ...
            if (showEditorActionContainerEl)
            {
                  ...
                  this.widgets.showEditorButton = Alfresco.util.createYUIButton(this, null, this.onShowEditorClick,
                  {
            ...
            this.widgets.editor = new YAHOO.widget.SimpleEditor(this.currentValueHtmlId,{
               width: '100%',
               animate: false,
               dompath: false,
               focusAtStart: true,
               toolbar: {
                  ...
                  buttons: [
                      { group: 'operators', label: this.msg("form.control.spel-editor.editor.operators"),
                          buttons: [
                              { type: 'push', label: '+', value: 'operator' },
                              { type: 'push', label: '-', value: 'operator' },
                              { type: 'push', label: '/', value: 'operator' },
                              { type: 'push', label: '*', value: 'operator' },
                              { type: 'push', label: '%', value: 'operator' },
                              ...
                              { type: 'push', label: '==', value: 'operator'},
                              { type: 'push', label: '!=', value: 'operator'},
                              { type: 'push', label: 'and', value: 'operator'},
                              { type: 'push', label: 'or', value: 'operator'},
                              { type: 'push', label: '=', value: 'operator' }
                             
                          ]},
                          { type: 'separator' },
                          { group: 'functions', label: this.msg("form.control.spel-editor.editor.functions"),
                          buttons: [
                                    
                                    { type: 'select', label: this.msg("form.control.spel-editor.editor.choose"), value: 'function',
                                        menu: [
      ...
      onSearch: function SpelEditor_onSearch()
      {
         ...
         if (searchTerm.length < this.options.minSearchTermLength)
         {
            ...
            {
               text: this.msg("form.control.spel-editor.search.enter-more", this.options.minSearchTermLength)
            });
         }
         else
         {
...
//         	 cleanRegExp = new RegExp("\.\?\[[a-z0-9A-Z\-]*\.toString\(\)\s*==\s*'(workspace://SpacesStore/[a-z0-9A-Z\-]*)'\s*\]\[0\]","g"),
//         	 value = "<br/>\n\n"+instance.options.currentValue.replace(cleanRegExp,"(\"$1\")")+"\n",
         	 
            var items = [],
            	 instance = this,
            	 regexp = new RegExp("(workspace://SpacesStore/[a-z0-9A-Z\-]*)"),
            	 value = "<br/>\n\n"+instance.options.currentValue+"\n",
					 brush = new SyntaxHighlighter.brushes.JScript();
					 brush.init({ toolbar: false });
            
         	
            items = regexp.exec(value);
						...
						successCallback : {
							fn : function(response) {
								...
								for ( var i = 0, il = items.length; i < il; i++) {
									item = items[i];
									value = value.replace(item.nodeRef, item.name);
									
								}
								Dom.get(this.id + "-currentValueDisplay").innerHTML = brush.getHtml(value);								
							},
							scope : this
						},
						failureCallback : {
      ...
      onParentTypeChanged: function SpelEditor_onParentTypeChanged(layer, args)
      {
         ...
         if ($hasEventInterest(this, args))
         {
            ...
            if (obj && obj.item)
            {
               this.widgets.itemTypeMenu.set("label", '<div><span class="item-icon"><img src="' + Alfresco.constants.URL_RESCONTEXT + 'components/form/images/ajax_anim.gif" width="16" height="16" alt="' + this.msg("message.please-wait") + '"></span><span class="item-name">' + $html(obj.item.name) + '</span></div>');
            ...
            var parentTypes = [
                               {
	                              ...
	                              },
	                              {
		                            ...
		                            },
		                            {
		                             	 ...
		                             	 parent:this.entityNodeRef,
		                             	 template: "dynamicCharachList.?[dynamicCharachTitle=='{item1}'][0].dynamicCharachValue"
		                            }
//	                             ,
//			                        {
//		                            	 name:"Functions",
//		                            	 type:"ArrayList",
//		                            	 values : ["$Value instanceof T($Type)","isMember('$String')"
//		                            	           ,"$String matches '$Regexp'","T($type)","$false ? '$trueExp' : '$falseExp'"
//		                            	           ,"$list.?[$property == $value]","$list.![$property]"],
//		                             	 template: "{item1}"
//			                        }
//	                             	                             
	                ];
            ...
            for(var i = 0; i< parentTypes.length; i++){
            	item = parentTypes[i];
            	 menuItem =  new YAHOO.widget.MenuItem(item.name,
                     {
            		 		value: item
                     });
      ...
      _createItemListControls: function SpelEditor__createItemListControls()
      {
         ...
         if (this.options.disabled === false)
         {
            ...
            [
               { key: "value", label: "Value", formatter: function (elCell, oRecord, oColumn, oData)
                {
               	var data = oRecord.getData();
               	
               	elCell.innerHTML = "<span class='" + data.cssClass
         					+ "' style='padding-left: 20px;' >" + data.name + "</span>";
               	
               } }
            ];

          
            this.widgets.dataTable1 = new YAHOO.widget.DataTable(this.editorId + "-itemList1", columnDefinitions, this.widgets.dataSource,
            {
            ...
            this.widgets.dataTable2 = new YAHOO.widget.DataTable(this.editorId + "-itemList2", columnDefinitions, this.widgets.dataSource,
            {
               MSG_EMPTY: this.msg("form.control.spel-editor.selected-items.empty"),
               initialLoad : false
            });
            ...
            this.widgets.dataTable1.doBeforeLoadData = function (sRequest, oResponse, oPayload) {
            	if(me.options.selectedParentType){
            		me.widgets.itemTypeMenu.set("label",me.options.selectedParentType.name);
            	}
               return true;
           };
           
           this.widgets.dataTable1.subscribe("cellClickEvent", function (oArgs) {
         	    ...
         	    if(me.options.selectedParentType.subType!=null){
         	   	 me.widgets.dataTable2.load({request:me._buildParamUrl(me.options.selectedParentType.subType)});
         	    } else {
         	   	  me.widgets.editor.execCommand('inserthtml', Lang.substitute(me.options.selectedParentType.template, {
             	   	 item1: data.value
             	    }));
         	    }
         	});
           ...
           this.widgets.dataTable2.subscribe("cellClickEvent", function (oArgs) {
       	    var target = oArgs.target,
       	    		oRecord = this.getRecord(target);
		 ...
		 * @returns {String}
		 */
      _buildParamUrl : function SpelEditor___buildUrl(type) {
      	
      	var q = YAHOO.util.Lang.substitute("?q={query}&page={page}&pageSize={pageSize}&className={className}", {
					query : "*",
					page : 1,
					className : type,
					pageSize :100
      ...
      onRefreshItemList : function SpelEditor_onRefreshItemListfunction (layer, args){
      	  if ($hasEventInterest(this, args))
           {
		      		this.widgets.dataTable1.load({request:this._buildParamUrl(this.options.selectedParentType.type)});
		      		this.widgets.dataTable2.deleteRows();
          }
      },
      ...
      _createResizer: function SpelEditor__createResizer()
      {
         if (!this.widgets.resizer)
         {
            var size = parseInt(Dom.get(this.editorId + "-body").offsetWidth, 10) - 2,
               heightFix = 0;
