...
(function()
{
   ...
   */
   ...
   beCPG.component.DockBar = function DockBar_constructor(htmlId, instanceName)
   {
      ...
      return beCPG.component.DockBar.superclass.constructor.call(this, "beCPG.component.DockBar", htmlId, ["button", "container", "datasource", "datatable", "animation"]);
   };

   YAHOO.extend(beCPG.component.DockBar, Alfresco.component.Base,
   {
      ...
      onReady: function DockBar_onReady()
      {
          ...
          * Create datatable
          */
         this.widgets.dockBarDataTable = new Alfresco.util.DataTable(
         {
            dataSource:
            {
               url: Alfresco.constants.PROXY_URI + "slingshot/doclib/doclist/product/node/alfresco/company/home?max=5&type=product&filter=favourites",
               config:
               {
                  responseSchema:
                  {
         ...
         var me = this,
            dataTable = this.widgets.dockBarDataTable.getDataTable(),
     ...
     renderCellDetail: function DockBar_renderCellDetail(elCell, oRecord, oColumn, oData)
     {
        ...
        else
        {
           var locn = record.location,
              ...
              desc +=    '<span class="item-separator"><a class="document-download" href="' + contentUrl + '"  title="' + this.msg( "actions.document.download") + '" tabindex="0">' +  this.msg("actions.document.download") + '</a></span>';
              desc +=    '<span class="item-separator"><a class="document-characts ' + CHARACT_EVENTCLASS + '" title="' + this.msg( "actions.document.viewEntityLists") + '" tabindex="0">' + this.msg("actions.document.viewEntityLists") + '</a></span>';
              desc += '</div>';
      ...
      onActionShowCharact: function DockBar_onActionShowCharact(row) {
      	...
      	var p_record = this.widgets.dockBarDataTable.getData(row),
