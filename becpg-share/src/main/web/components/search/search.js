...
(function() {
	...
	Alfresco.Search = function(htmlId) {
		Alfresco.Search.superclass.constructor.call(this, "Alfresco.Search", htmlId, [ "button", "container",
	...
	YAHOO
	      .extend(
	            ...
	            Alfresco.component.Base,
	            {
	               ...
	               /**
						 * Whether the search was over all sites or just the current
						 * one
						 */
	               searchAllSites : true,
	               ...
	               /**
						 * True if there are more results than the ones listed in the
						 * table.
						 */
	               ...
	               _setupDataTable : function Search_setupDataTable() {
		               ...
		               renderCellThumbnail = function Search_renderCellThumbnail(elCell, oRecord, oColumn, oData) {
			               ...
			               if (dataType === "document") {
				               var viewUrl = Alfresco.constants.PROXY_URI_RELATIVE + "api/node/content/"
				                     + oRecord.getData("nodeRef").replace(":/", "") + "/" + oRecord.getData("name");
				               html = '<div class="action-overlay">' + '<a href="' + encodeURI(viewUrl)
				                     + '" target="_blank"><img title="' + $html(me.msg("label.viewinbrowser")) + '" src="'
				                     + Alfresco.constants.URL_RESCONTEXT
				                     + 'components/search/images/view-in-browser-16.png" width="16" height="16"/></a>'
				                     + '<a href="' + encodeURI(viewUrl + "?a=true")
				                     + '" style="padding-left:4px" target="_blank"><img title="'
				                     + $html(me.msg("label.download")) + '" src="' + Alfresco.constants.URL_RESCONTEXT
				                     + 'components/search/images/download-16.png" width="16" height="16"/></a>' + '</div>'
				                     + html;
			               }
		               ...
		               renderCellDescription = function Search_renderCellDescription(elCell, oRecord, oColumn, oData) {
			               ...
			               if (type === "entity" || type === "document" || type === "folder") {
				               ...
				               if (site) {
					               ...
					               desc += '<div class="details">' + me.msg("message.infolderpath") + ': <a href="'
					                     + me._getBrowseUrlForFolderPath(path, site) + '">' + $html('/' + path)
					                     + '</a></div>';
				               } else {
					               if (path) {
						               desc += '<div class="details">' + me.msg("message.infolderpath") + ': <a href="'
						                     + me._getBrowseUrlForFolderPath(path) + '">' + $html(path) + '</a></div>';
					               }
				               }
			               }
	               ...
	               _getBrowseUrlForRecord : function Search__getBrowseUrlForRecord(record) {
		               ...
		               var name = record.getData("name"), type = record.getData("type"), site = record.getData("site"), path = record
		                     .getData("path");
			               ...
			               if (path !== null) {
				               if (site) {
					               url = "documentlibrary?path="
					                     + encodeURIComponent(this._buildSpaceNamePath(path.split("/"), name));
				               } else {
					               url = "repository?path="
					                     + encodeURIComponent(this._buildSpaceNamePath(path.split("/").slice(2), name));
				               }
			               }
			               break;
		               }
		               ...
		               case "calendarevent": {
			               url = record.getData("container") + "?date="
			                     + Alfresco.util.formatDate(record.getData("modifiedOn"), "yyyy-mm-dd");
	               ...
	               /**
						 ...
						 * 
						 * @param path
						 *           {string} folder path For a site relative item
						 *           this can be empty (root of doclib) or any path -
						 *           without a leading slash For a repository item,
						 *           this can never be empty - but will contain
						 *           leading slash and Company Home root
						 */
	               _getBrowseUrlForFolderPath : function Search__getBrowseUrlForFolderPath(path, site) {
		               var url = null;
		               if (site) {
			               url = Alfresco.constants.URL_PAGECONTEXT + "site/" + site.shortName + "/documentlibrary?path="
			                     + encodeURIComponent('/' + path);
		               } else {
			               url = Alfresco.constants.URL_PAGECONTEXT + "repository?path="
			                     + encodeURIComponent('/' + path.split('/').slice(2).join('/'));
		               }
		               return url;
	               },
	               ...
	               /**
						 * DEFAULT ACTION EVENT HANDLERS Handlers for standard events
						 * fired from YUI widgets, e.g. "click"
						 */

	               /**
						 * Perform a search for a given tag The tag is simply handled
						 * as search term
						 */
	               ...
	               refreshSearch : function Search_refreshSearch(args) {
		               ...
		               if (searchQuery.length !== 0) {
			               ...
			               // other options such as tag, terms, all sites, repo
								// etc. are trumped
			               url += "&q=" + searchQuery;
		               } else {
	               ...
	               /**
						 * Updates search results list by calling data webscript with
						 * current site and query term
						 * 
	               ...
	               _performSearch : function Search__performSearch(args) {
		               var searchTerm = YAHOO.lang.trim(args.searchTerm), searchTag = YAHOO.lang.trim(args.searchTag), searchAllSites = args.searchAllSites, searchRepository = args.searchRepository, searchSort = args.searchSort;

		               if (this.options.searchQuery.length === 0 && searchTag.length === 0
		               ...
		               function successHandler(sRequest, oResponse, oPayload) {
			               ...
			               this.widgets.dataTable.onDataReturnInitializeTable.call(this.widgets.dataTable, sRequest,
			                     oResponse, oPayload);
		               ...
		               this.widgets.dataSource.sendRequest(this._buildSearchParams(searchRepository, searchAllSites,
		                     searchTerm, searchTag, searchSort), {
	               ...
	               _buildSearchParams : function Search__buildSearchParams(searchRepository, searchAllSites, searchTerm,
		               ...
		               var params = YAHOO.lang
		                     .substitute(
		                           "site={site}&term={term}&tag={tag}&maxResults={maxResults}&sort={sort}&query={query}&repo={repo}&metadataFields={metadataFields}&page={page}&pageSize={pageSize}",
		                           {
		                              site : encodeURIComponent(site),
		                              repo : (searchRepository || this.options.searchQuery.length !== 0).toString(), // always
																																							// search
																																							// entire
																																							// repo
																																							// with
