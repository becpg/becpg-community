                  ...
                  onReady : function AutoCompletePicker_onReady() {
                     ...
                     if (me.options.mode != "view" && !this.options.readOnly) {
                        ...
                        if (this.options.isLocalProxy) {
                           oDS = new YAHOO.util.XHRDataSource(Alfresco.constants.URL_SERVICECONTEXT + me.options.dsStr);
                        } else {
                        ...
                        if (me.options.showToolTip) {
                           ...
                           previewTooltip.contextTriggerEvent
                                 .subscribe(function(type, args) {
                                    var context = args[0];
                                    var nodeRef = context.id.split('ac-choice-'+me.fieldHtmlId+'-')[1].replace(":/", "");
                                    this.cfg
                                          .setProperty(
                                                "text",
                              ...
                              var oACPagination = new YAHOO.widget.Paginator(
                                    {
                                       ...
                                       pageReportTemplate : me.msg("autocomplete.pagination.template.page-report"),
                                       previousPageLinkLabel : me.msg("autocomplete.pagination.previousPageLinkLabel"),
                                       nextPageLinkLabel : me.msg("autocomplete.pagination.nextPageLinkLabel")
                                    });
                        ...
                        if (this.options.mode == "view" || this.options.readOnly) {
                           for ( var key in items) {
                              ...
                              if (this.options.targetLinkTemplate != null) {
                                 ...
                                 link = Alfresco.util.siteURL(Lang.substitute(this.options.targetLinkTemplate, item), {
                                    site : item.site
                                 });
