...
export REMOTE_SERVER=http://admin:becpg@82.237.72.111:8080/alfresco/service/becpg/remote/entity
   ...
   then
		...
		do
		   ...
		   echo "Getting $nodeRef"; 
			wget --quiet --http-user=admin --http-password=becpg  --header=Accept-Charset:iso-8859-1,utf-8 --header=Accept-Language:en-us -O entity.xml $REMOTE_SERVER?nodeRef=$nodeRef
			curl --silent -H "Content-Type: application/xml"  -X PUT --data @entity.xml  $LOCAL_SERVER?callback=$REMOTE_SERVER > nodeRef
			
			entityNodeRef=`cat nodeRef`
			
			# DATA
			wget --quiet  --http-user=admin --http-password=becpg  --header=Accept-Charset:iso-8859-1,utf-8 --header=Accept-Language:en-us -O data.xml $REMOTE_SERVER/data?nodeRef=$nodeRef
