                        ...
                        var fnDrawGantt = function PL_onReady_fnDrawGantt()
                                    {
                                        ...
                                        if (recordSet.getLength() != 0)
                                        {
                                            ...
                                            g.setDateInputFormat("shortDate");
                                            g.setDateDisplayFormat("shortDate");
                                            g.setCaptionType('Caption'); // affichage d'un message personnalisée en face des tâches
                                            g.setShowRes(0); // Masque les ressources
                                            g.setShowDur(0); // Masque la durée
                                            g.setShowComp(0); //Masque l'avancement
                                            var start = new Date();
                                            for (var i = 0; i < recordSet.getLength(); i++)
                                            {
                                                var oRecord = recordSet.getRecord(i);
                                                ...
                                                g.AddTaskItem(new JSGantt.TaskItem(taskId, this.getDeliverableTitle(deliverable,
                                                        this.options.entityNodeRef), start,
                                                        end,color, null, tlIsMilestone ? 1 : 0,
                                                        taskOwner, tlPercent, pGroup, pParent, 1, precTaskIds));
                                            }
