package fr.becpg.repo.googledocs;
...
/**
 * Google docs integration service implementation with oauth
 */
public class OAuthGoogleDocsServiceImpl extends TransactionListenerAdapter implements GoogleDocsService, GoogleDocsModel {
	/** Log */
	private static Log logger = LogFactory.getLog(OAuthGoogleDocsServiceImpl.class);
	...
	/**
	 * List of supported GoogleDoc supported mimetypes. Taken from list found at
	...
	private List<String> supportedMimetypes = Arrays.asList("text/csv", "text/tab-separated-values", "text/html", "application/msword",
			"application/vnd.openxmlformats-officedocument.wordprocessingml.document", "application/x-vnd.oasis.opendocument.spreadsheet",
			"application/vnd.oasis.opendocument.text", "application/rtf", "application/vnd.sun.xml.writer", "text/plain", "application/vnd.ms-excel",
			"application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", "application/pdf", "application/vnd.ms-powerpoint", "image/x-wmf");
	...
	public MediaService getMediaService(String serviceName) {
		...
		try {
			...
			logger.error(e, e);
			throw new GoogleDocsServiceInitException("Cannot open oAuth session");
		}
	...
	/**
	 * @param spreadsheetDownloadUrl
	 *            root spreadsheet download URL
	 */
	...
	/**
	 * @param spreadSheetServiceName
	 *            GDoc spread sheet service name
	 */
	...
	public void createGoogleDoc(NodeRef nodeRef, GoogleDocsPermissionContext permissionContext) {
		...
		// TODO should be checking to make sure this doesn't already have an
	...
	public void deleteGoogleResource(NodeRef nodeRef) {
		...
		if (nodeService.hasAspect(nodeRef, ASPECT_GOOGLERESOURCE) == true) {
			...
			if (entry != null) {
				// Mark the resource for deletion upon completion of the
				// transaction
				...
				if (document != null) {
					...
					if (docType.equals(TYPE_DOCUMENT) || docType.equals(TYPE_PRESENTATION)) {
						...
						if (fileExtension.equals("csv") || fileExtension.equals("tsv")) {
							buffer.append("&gid=0"); // gid=0 will download only
														// the first sheet
						}
	...
	private DocumentListEntry createGoogleDocument(String name, String mimetype, DocumentListEntry parentFolder, InputStream is) {
		...
		try {
			...
			if (MediaType.XLS.getMimeType().equals(mimetype) == true || MediaType.XLSX.getMimeType().equals(mimetype) == true
			...
			} else if (MediaType.PPS.getMimeType().equals(mimetype) == true || MediaType.PPT.getMimeType().equals(mimetype) == true) {
				if (logger.isDebugEnabled() == true) {
...
//	}
	 ...
	 * 
	 * @param resourceId
	 *            resource id of created resource
	 */
