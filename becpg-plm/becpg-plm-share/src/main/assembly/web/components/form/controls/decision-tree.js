        ...
        .augmentObject(
            beCPG.component.DecisionTree.prototype,
            {
                ...
                onAfterFormRuntimeInit: function(layer, args) {
                    if (!this.options.disabled && this.formRuntime == null && this.id.indexOf(args[1].runtime.formId.replace("-form", "")) > -1) {
                            ...
                            for (var j = 0; j < question.choices.length; j++) {
                                ...
                                if ((choice.list != null && !choice.checkboxes && Dom.get(this.id + "-select_" + question.id + '_' + choice.id).value != null)
                                    ...
                                    if (choice.list != null) {
                                        ...
                                        ret.push({
                                            qid: question.id, cid: choice.id, listOptions:
                                                value
                                        });
                                    ...
                                    if (choice.comment) {
                                        ...
                                        if (choice.commentLabel && choice.commentLabel.length > 0 && Dom.get(this.id + '-comment_' + question.id + '-label') != null) {
                                            var labelEl = Dom.get(this.id + '-comment_' + question.id + '-label');
                                            var labelVal = choice.commentLabel == "hidden" ? "" : choice.commentLabel;
                                            if ((labelVal || "") != "") {
                ...
                insertComment: function(questionId, choice) {
                    ...
                    if (existingInput && existingInput.getAttribute('data-choice-id') === choice.id) return;

                    var htmlForm = "";
                    var currentValue = this.getCurrentValueComment(questionId);

                    if (choice.label && choice.label != "hidden") {
                        htmlForm += '<label id="' + labelId + '" for="' + inputId + '">' + this.msg("form.control.decision-tree." + this.options.prefix + "." + questionId + ".comment") + ':</label>';
                    }

                    if (this.options.disabled) {
                        htmlForm += '<span id="' + inputId + '" data-choice-id="' + choice.id + '">' + currentValue + '</span>';
                    } else {
                        var baseAttrs = 'data-choice-id="' + choice.id + '" tabindex="0" id="' + inputId + '" class="' + COMMENT_EVENTCLASS + '" name="--comment_' + this.id + questionId + '"';

                        if (choice.commentType === "textarea") {
                            htmlForm += '<textarea ' + baseAttrs + '>' + currentValue + '</textarea>';
                        } else if (choice.commentType && INPUTS[choice.commentType]) {
                            var inputAttrs = baseAttrs;
                            var inputConfig = INPUTS[choice.commentType];
                            for (var attrKey in inputConfig) {
                                if (inputConfig.hasOwnProperty(attrKey)) {
                                    inputAttrs += ' ' + attrKey + '="' + inputConfig[attrKey] + '"';
                                }
                            }
                            inputAttrs += ' value="' + currentValue + '"';
                            htmlForm += '<input ' + inputAttrs + ' />';
                        } else {
                            // Fallback to text input if commentType is not recognized or not specified
                            htmlForm += '<input type="text" ' + baseAttrs + ' value="' + currentValue + '" />';
                        }
                    }
