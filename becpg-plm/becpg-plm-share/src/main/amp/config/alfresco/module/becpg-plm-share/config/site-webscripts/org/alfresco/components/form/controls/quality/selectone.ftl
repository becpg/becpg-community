      ...
      <#if field.control.params.options?? && field.control.params.options != "">
         <select id="${fieldHtmlId}" name="${field.name}" tabindex="0"
               ...
               <#list field.control.params.options?split(optionSeparator) as nameValue>
                  ...
                  <#else>
                     <#assign choice=nameValue?split(labelSeparator)>
                     <option value="${choice[0]?html}"<#if choice[0] == fieldValue?string || (fieldValue?is_number && fieldValue?c == choice[0])> selected="selected"</#if>>${msg(choice[1])?html}</option>
                  </#if>
               </#list>
         </select>
		         ...
		         var formUI = args[1].component,
		            formsRuntime = args[1].runtime;
		         	YAHOO.util.Event.addListener("${fieldHtmlId}", "change", function(){
			         	formsRuntime.validations = [];
			         	formsRuntime.updateSubmitElements();
		         	});
         	}, this);
         </script>
