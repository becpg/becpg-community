...
(function() {
	...
	YAHOO.extend(Alfresco.AdvancedSearch, Alfresco.component.Base, {
		...
		options : {
			 ...
			 * "cm:content", label: "Content", description: "All types of
			 * content" }
			 * 
				...
				if (savedQuery.datatype) {
					for ( var f in this.options.searchForms) {
						...
						if (form.type === savedQuery.datatype) {
							// found previous form datatype - use as first form
							// to display
							defaultForm = form;
							break;
						}
		 ...
		 * @param repopulate
		 *            {boolean} If true, repopulate form instance based on
		 *            supplied data
		 */
						...
						if (savedValue !== undefined) {
							if (element.type === "checkbox" || element.type === "radio") {
								...
								if (cntrl) {
									...
									// probably date range - just set value and
									// control will handle it
									element.value = savedValue;
								}
							}
							// beCPG : clear the value, otherwise value is
							// keeped in a hidden field and we don't understand
							// the results
							// TODO : assoc, now we clear saved value, but we
							// should rendered it instead of clearing it
							else if (name.indexOf("assoc_") != 0) {
								element.value = savedValue;
							}

							// reverse value setting doesn't work with
							// checkboxes or multi-select boxes because of the
							// hidden field used to store the underlying field
							// value
							if (element.type === "hidden") {
								// hidden fields could be a part of a checkbox
								// or similar in the Forms runtime
								// so look if there is a entry element attached
								// this hidden field and set the value
								...
								if (cntrl) {
									...
									case "checkbox":
										...
										break;
									default: // "select-multiple" - and
												// potentially others following
												// the same pattern
										cntrl.value = savedValue;
										break;
									}
		...
		onSearchClick : function ADVSearch_onSearchClick(e, obj) {
			...
			if(this.currentForm!=null ){
				...
				// build and execute url for search page
				 url = YAHOO.lang.substitute(Alfresco.constants.URL_PAGECONTEXT + "{site}search?t={terms}&q={query}&r={repo}", {
				 ...
				 url = YAHOO.lang.substitute(Alfresco.constants.URL_PAGECONTEXT + "{site}search?t={terms}&r={repo}", {
					site : (this.options.siteId.length !== 0 ? ("site/" + this.options.siteId + "/") : ""),
					terms : encodeURIComponent(Dom.get(this.id + "-search-text").value),
					repo : this.options.searchRepo.toString()
				});
		...
		/**
		 * Event handler called when the "afterFormRuntimeInit" event is
		 * received
		 */
