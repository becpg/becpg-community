            ...
            .extend(
                    ...
                    Alfresco.Search,
                    {
                        ...
                        onReady : function Search_onReady()
                        {
                            ...
                            for ( var m in menuItems)
                            {
                                if (menuItems[m].value === this.options.initialSort)
                                {
                                    this.widgets.sortButton.set("label", this.msg("label.sortby", menuItems[m].cfg
                                            .getProperty("text")) + " " + Alfresco.constants.MENU_ARROW_SYMBOL);
                                    break;
                                }
                            }
                            ...
                            if (Dom.get(this.id + "-export-menubutton"))
                            {
                                ...
                                for ( var m in menuItems)
                                {
                                    ...
                                    {
                                        this.widgets.exportButton
                                                .set(
                                                        "label",
                                                        this.msg("label.export-search", menuItems[m].cfg
                                                                .getProperty("text")) + " " + Alfresco.constants.MENU_ARROW_SYMBOL);
                                        break;
                                    }
                                }
                        ...
                        _setupDataTable : function Search_setupDataTable()
                        {
                             ...
                             *            {object|string}
                             */
                            renderCellDescription = function Search_renderCellDescription(elCell, oRecord, oColumn,
                                    oData)
                            {
                                ...
                                if (oRecord.getData("size") !== -1)
                                {
                                    ...
                                    desc += ' <span class="meta">' + Alfresco.util.formatFileSize(oRecord
                                            .getData("size")) + '</span>';
                                }
                                if (oRecord.getData("modifiedBy"))
                                {
                                    desc += ' ' + me.msg("message.modifiedby");
                                    desc += ' <a href="' + Alfresco.constants.URL_PAGECONTEXT + 'user/' + encodeURI(oRecord
                                            .getData("modifiedByUser")) + '/profile">' + $html(oRecord
                                            .getData("modifiedBy")) + '</a>';
                                }
                                desc += ' ' + me.msg("message.modifiedon") + ' <span class="meta">' + Alfresco.util
                                        .formatDate(oRecord.getData("modifiedOn")) + '</span>';
                                desc += '</div>';
                                ...
                                if (itemData != null)
                                {
                                    ...
                                    for (key in itemData)
                                    {
                                        ...
                                        if (item.displayValue != null && item.displayValue.length > 0)
                                        {
                                            ...
                                            desc += '<span id="' + me.id + '|' + $html(propName) + '|' + $html(item.value) + '|' + $html(itemType) + '" class="searchByProp" ><a class="search-prop" href="#">' + $html(item.displayValue) + '</a></span>';
                                            desc += '</div>';
                                        }
                                        else if (item.length > 0)
                                        {
                                            for (jj in item)
                                            {
                                                ...
                                                desc += '<span id="' + me.id + '|' + $html(propName) + '|' + $html(item[jj].value) + '|' + $html(itemType) + '" class="searchByProp" ><a class="search-prop" href="#">' + $html(item[jj].displayValue) + '</a></span>';
                            ...
                            this.widgets.dataTable = new YAHOO.widget.DataTable(this.id + "-results",
                            ...
                            this.widgets.dataTable.doBeforeLoadData = function Search_doBeforeLoadData(sRequest,
                                    oResponse, oPayload)
                            {
                                if (oResponse.error)
                                {
                        ...
                        exportSearch : function Search_exportSearch(args)
                        {
                            ...
                            // call webscript
                            var url = Alfresco.constants.PROXY_URI + "becpg/report/exportsearch/" + args.reportTpl
                                    .replace("://", "/") + "/" + encodeURI(args.reportFileName);
                         ...
                         */
                        _buildSearchParams : function Search__buildSearchParams(searchRepository, searchAllSites,
