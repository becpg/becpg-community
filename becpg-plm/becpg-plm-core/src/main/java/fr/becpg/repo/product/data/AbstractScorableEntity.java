...
public abstract class AbstractScorableEntity extends BeCPGDataObject implements ScorableEntity {
	...
	@Override
	public boolean mergeRequirements(boolean removeItemsWithoutFormulationId) {

		boolean hasChanged = false;
		
		List<ReqCtrlListDataItem> reqCtrlList = getReqCtrlList();
		...
		if (reqCtrlList != null) {
			...
			for (Map.Entry<String, ReqCtrlListDataItem> dbKV : dbReqCtrlList.entrySet()) {
				if (!newReqCtrlList.containsKey(dbKV.getKey())) {
					...
					if ((dbKV.getValue().getFormulationChainId() == null && removeItemsWithoutFormulationId)
					...
					if ((dbKV.getValue().getReqType() != null || newReqCtrlListDataItem.getReqType() != null) && dbKV.getValue().getReqType() != newReqCtrlListDataItem.getReqType()) {
					...
					if (dbKV.getValue().getReqMaxQty() != null && !dbKV.getValue().getReqMaxQty().equals(newReqCtrlListDataItem.getReqMaxQty())
							|| newReqCtrlListDataItem.getReqMaxQty() != null && !newReqCtrlListDataItem.getReqMaxQty().equals(dbKV.getValue().getReqMaxQty())) {
					...
					if (dbKV.getValue().getSources() != null && !dbKV.getValue().getSources().equals(newReqCtrlListDataItem.getSources())
							|| newReqCtrlListDataItem.getSources() != null && !newReqCtrlListDataItem.getSources().equals(dbKV.getValue().getSources())) {
					...
					if (dbKV.getValue().getCharact() != null && !dbKV.getValue().getCharact().equals(newReqCtrlListDataItem.getCharact())
							|| newReqCtrlListDataItem.getCharact() != null && !newReqCtrlListDataItem.getCharact().equals(dbKV.getValue().getCharact())) {
						dbKV.getValue().setCharact(newReqCtrlListDataItem.getCharact());
						hasChanged = true;
					}
					if ((dbKV.getValue().getReqDataType() != null || newReqCtrlListDataItem.getReqDataType() != null) && dbKV.getValue().getReqDataType() != newReqCtrlListDataItem.getReqDataType()) {
