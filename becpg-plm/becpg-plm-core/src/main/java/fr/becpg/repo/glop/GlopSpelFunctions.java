 ...
 * 
 * <h1>Usage</h1>
 * In the composition list, add a dynamic formula making a call to {@code @glop.optimize()}.
 * The first parameter should be {@code #this}. The second should be an object of the following structure:
 * <ul>
 * <li>{@code target} is an object describing the target function and is comprised of:
 * <ul>
 * <li>{@code var}, the data item to optimize</li>
 * <li>{@code task}, the optimization task, must be "min" or "max"
 * </ul></li>
 * <li>{@code constraints} is a set of constraints, each one being objects with the following structure:
 * <ul>
 * <li>{@code var}, either the data item of the constraint or the name tag of a special constraint (see below)</li>
 ...
 * </ul>
 * 
 * The {@code min} and {@code max} values may be floating-point numbers or the strings {@code "inf"} and {@code "-inf"}.
 * 
 * <p>The result is an object with the following structure:
 * <ul>
 * <li>{@code coefficients}: a map where the keys are the component names and the values are their quantity in the optimized recipe</li>
 * <li>{@code value}: the value that the target function takes under this calculated recipe</li>
 * <li>{@code status}: "optimized" if the solution is optimal, "feasible" if it satisfies the constraints but isn't guaranteed optimal</li>
 * </ul>
 * 
 * <h1>Types of constraint</h1>
 * Data constraints are calculated from instances of sub-classes of {@link fr.becpg.repo.repository.model.BeCPGDataObject}
 * that are referenced in the product data. The types currently supported are:
 * <ul>
 * <li>{@link fr.becpg.repo.product.data.productList.CompoListDataItem}, through the SpEL expression {@code compo["ref"]}</li>
 * <li>{@link fr.becpg.repo.product.data.productList.NutListDataItem}, through the SpEL expression {@code nut["ref"]}</li>
 * <li>{@link fr.becpg.repo.product.data.productList.CostListDataItem}, through the SpEL expression {@code cost["ref"]}</li>
 * <li>{@link fr.becpg.repo.product.data.productList.IngListDataItem}, through the SpEL expression {@code ing["ref"]}</li>
 * <li>{@link fr.becpg.repo.product.data.productList.AllergenListDataItem}, through the SpEL expression {@code allergen["ref"]}</li>
 * <li>{@link fr.becpg.repo.product.data.productList.PackagingListDataItem}, through the SpEL expression {@code packaging["ref"]}</li>
 * </ul>
 * 
 * Special constraints are constraints that require more than one data item to create.
 * Currently, the only special constraint is {@code "total_qty"} which evaluates as the total quantity of components in the product.
 *
 * @author pierrecolin
...
public class GlopSpelFunctions implements CustomSpelFunctions {
	...
	public class GlopSpelFunctionsWrapper {
		...
		public Map<String, Serializable> optimize(ProductData product, Map<String, ?> problem) {
			...
			BeCPGDataObject targetItem = (BeCPGDataObject) target.get("var");
			...
			for (Object objConstraint: constraints) {
				...
				if (objConstraintItem instanceof BeCPGDataObject) {
					BeCPGDataObject constraintItem = (BeCPGDataObject) objConstraintItem;
