					...
					try {
										...
										if ((jsonValue.length() > LargeTextHelper.TEXT_SIZE_LIMIT)) {
											...
											productData.getReqCtrlList()
													.add(new ReqCtrlListDataItem(null, RequirementType.Info,
															MLTextHelper.getI18NMessage("message.formulate.formula.toolong",
						...
						productData.getReqCtrlList()
								.add(new ReqCtrlListDataItem(null, RequirementType.Forbidden,
										MLTextHelper.getI18NMessage("message.formulate.formula.error", dynamicCharactListItem.getTitle(),
