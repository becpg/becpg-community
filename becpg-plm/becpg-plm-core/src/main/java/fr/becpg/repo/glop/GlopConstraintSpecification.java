...
public class GlopConstraintSpecification implements Serializable {
	...
	public String toString() {
		if (isSpecial()) {
			return "GlopConstraintSpecification [special=" + special + ", minValue=" + minValue
				+ ", maxValue=" + maxValue + "]";
		} else {
			return "GlopConstraintSpecification [data=" + data + ", minValue=" + minValue
					+ ", maxValue=" + maxValue + "]";
		}
	}

	private BeCPGDataObject data;
	 ...
	 * 
	 * @param data the data item from which to generate a constraint
	 * @param minValue the lowest accepted value for the item
	 * @param maxValue the highest accepted value for the item
	 * @throws IllegalArgumentException if {@code minValue > maxValue}
	 */
	public GlopConstraintSpecification(BeCPGDataObject data, Double minValue, Double maxValue) throws IllegalArgumentException {
	 ...
	 * 
	 * @param special the special constraint identifier
	 ...
	 */
	public BeCPGDataObject getData() {
		return data;
	}
	...
	/**
	 * Adds a constraint to the problem specification if it's a special constraint. Intended to be used by {@link fr.becpg.repo.glop.impl.GlopServiceImpl}.
	 * 
	 * @param specialContributions the map of special constraints
	 * @param compoListDataItem the component under scrutiny
	 * @param variables the set of variables
	 */
	public void fillIfSpecial(Map<String, Map<CompoListDataItem, Double>> specialContributions, CompoListDataItem compoListDataItem, Set<CompoListDataItem> variables) {
		if (isSpecial()) {
			Double coef = null;
	...
	/**
	 * Generates a {@link org.json.JSONObject} representation of the constraint. Intended to be used by {@link fr.becpg.repo.glop.impl.GlopServiceImpl}.
	 * 
	 * @param constraintContributions the map of data constraints
	 * @param specialContributions the map of special constraints
	 * @return the JSON representation of the constraint
	 * @throws JSONException if the creation failed
	 */
	public JSONObject toJson(Map<BeCPGDataObject, Map<CompoListDataItem, Double>> constraintContributions, Map<String, Map<CompoListDataItem, Double>> specialContributions) throws JSONException {
	...
	/**
	 * Initializes the constraint in the correct map. Intended to be used by {@link fr.becpg.repo.glop.impl.GlopServiceImpl}.
	 * 
	 * @param constraintContributions the map of data constraints
	 * @param specialContributions the map of special constraints
	 */
	public void initContributions(Map<BeCPGDataObject, Map<CompoListDataItem, Double>> constraintContributions, Map<String, Map<CompoListDataItem, Double>> specialContributions) {
		if (isSpecial()) {
