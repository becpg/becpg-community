...
public class ToxicologyServiceImpl implements ToxicologyService {
	...
	public void updateIngredient(NodeRef ingNodeRef) {
		...
		for (NodeRef toxNodeRef : toxList) {
			...
			String newSystemicValues = ToxHelper.appendToxValue(previousSystemicValues, toxName, systemicValue);
			if (logger.isDebugEnabled()) {
				logger.debug("Systemic value computed: " + systemicValue + ", updated values: " + newSystemicValues);
			}
			
			Double maxValue = computeMaxValue(ingNodeRef, toxNodeRef, toxName, systemicValue);
			String newMaxValues = ToxHelper.appendToxValue(previousMaxValues, toxName, maxValue);
			if (logger.isDebugEnabled()) {
				logger.debug("Max value computed: " + maxValue + ", updated values: " + newMaxValues);
			}
	...
	private void updateIngFromTox(NodeRef ingNodeRef, NodeRef toxNodeRef) {
		...
		String toxName = extractToxName(toxNodeRef);
		String newSystemicValues = null;
		String newMaxValues = null;
		if (Boolean.TRUE.equals(nodeService.getProperty(toxNodeRef, BeCPGModel.PROP_IS_DELETED))) {
			if (logger.isDebugEnabled()) {
				logger.debug("Tox is deleted, removing from ingredient: " + ingNodeRef);
			}
			newSystemicValues = ToxHelper.removeToxValue(currentSystemicValues, toxName);
			newMaxValues = ToxHelper.removeToxValue(currentMaxValues, toxName);
		} else {
			if (logger.isDebugEnabled()) {
				logger.debug("Updating ingredient: " + ingNodeRef + " with tox: " + toxName);
			}
			Double systemicValue = computeSystemicValue(ingNodeRef, toxNodeRef, toxName);
			if (systemicValue == null) {
				newSystemicValues = ToxHelper.removeToxValue(currentSystemicValues, toxName);
			} else {
				newSystemicValues = ToxHelper.appendToxValue(currentSystemicValues, toxName, systemicValue);
			}
			Double maxValue = computeMaxValue(ingNodeRef, toxNodeRef, toxName, systemicValue);
			if (maxValue == null) {
				newMaxValues = ToxHelper.removeToxValue(currentMaxValues, toxName);
