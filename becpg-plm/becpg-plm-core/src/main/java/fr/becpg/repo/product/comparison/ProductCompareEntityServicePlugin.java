...
public class ProductCompareEntityServicePlugin extends DefaultCompareEntityServicePlugin {
	...
	protected void multiLevelComparison(QName dataListType, String charactName, String pivotKey, NodeRef entity1NodeRef, NodeRef entity2NodeRef,
		...
		if (dataListType.equals(PLMModel.TYPE_COMPOLIST)) {
			addCompoListProps(comparisonMap, dataListType, charactName, pivotKey, entity1NodeRef, nbEntities, comparisonPosition, totalQty, null,
					false, 0, 1);
			addCompoListProps(comparisonMap, dataListType, charactName, pivotKey, entity2NodeRef, nbEntities, comparisonPosition, totalQty, null,
					true, 1, 1);
		} else if (dataListType.equals(PLMModel.TYPE_PACKAGINGLIST)) {
			addPackagingListProps(comparisonMap, dataListType, charactName, pivotKey, entity1NodeRef, nbEntities, comparisonPosition, totalQty, null,
					false, 0, 1);
			addPackagingListProps(comparisonMap, dataListType, charactName, pivotKey, entity2NodeRef, nbEntities, comparisonPosition, totalQty, null,
					true, 1, 1);
		} else if (dataListType.equals(MPMModel.TYPE_PROCESSLIST)) {
			addProcessListProps(comparisonMap, dataListType, charactName, pivotKey, entity1NodeRef, nbEntities, comparisonPosition, totalQty, null,
					false, 0, 1);
			addProcessListProps(comparisonMap, dataListType, charactName, pivotKey, entity2NodeRef, nbEntities, comparisonPosition, totalQty, null,
					true, 1, 1);
		}
	}

	private void addCompoListProps(Map<String, CompareResultDataItem> comparisonMap, QName dataListType, String charactName, String pivotKey,
			NodeRef entityNodeRef, int nbEntities, int comparisonPosition, double[] totalQty, ProductData baseProduct, boolean swap, int position,
			int level) {
		...
		if (entityNodeRef != null) {
			...
			String previousQtyForProductStr = getCurrentValue(comparisonMap, dataListType, charactName, PLMModel.PROP_COMPOLIST_QTY_FOR_PRODUCT,
					position);
			...
			for (CompoListDataItem compoItem : product.getCompoList()) {
				addCompoListProps(comparisonMap, dataListType, attributeExtractorService.extractPropName(compoItem.getProduct()),
						compoItem.getNodeRef().toString(), compoItem.getNodeRef(), nbEntities, comparisonPosition, totalQty, baseProduct, swap,
						position, level + 1);
			}
		}
	}

	private void addPackagingListProps(Map<String, CompareResultDataItem> comparisonMap, QName dataListType, String charactName, String pivotKey,
			NodeRef entityNodeRef, int nbEntities, int comparisonPosition, double[] totalQty, ProductData baseProduct, boolean swap, int position,
			int level) {
		...
		if (entityNodeRef != null) {
			...
			String previousQtyForProductStr = getCurrentValue(comparisonMap, dataListType, charactName, PLMModel.PROP_PACKAGINGLIST_QTY_FOR_PRODUCT,
					position);
			...
			extractProps(comparisonMap, dataListType, charactName, pivotKey, PLMModel.PROP_PACKAGINGLIST_UNIT, value, nbEntities, comparisonPosition,
					swap);

			// Extract props from datalist items
			for (PackagingListDataItem packagingItem : product.getPackagingList()) {
				addPackagingListProps(comparisonMap, dataListType, attributeExtractorService.extractPropName(packagingItem.getProduct()),
						packagingItem.getNodeRef().toString(), packagingItem.getNodeRef(), nbEntities, comparisonPosition, totalQty, baseProduct,
						swap, position, level + 1);
			}
		}
	}

	private void addProcessListProps(Map<String, CompareResultDataItem> comparisonMap, QName dataListType, String charactName, String pivotKey,
			NodeRef entityNodeRef, int nbEntities, int comparisonPosition, double[] totalQty, ProductData baseProduct, boolean swap, int position,
			int level) {
		...
		if (entityNodeRef != null) {
			...
			if (item.getResource() != null) {
				...
				String previousQtyForProductStr = getCurrentValue(comparisonMap, dataListType, charactName, MPMModel.PROP_PL_QTY_FOR_PRODUCT,
						position);
				...
				extractProps(comparisonMap, dataListType, charactName, pivotKey, MPMModel.PROP_PL_UNIT, value, nbEntities, comparisonPosition, swap);
	...
	private void extractProps(Map<String, CompareResultDataItem> comparisonMap, QName dataListType, String charactName, String pivotKey,
			QName property, String value, int nbEntities, int comparisonPosition, boolean swap) {

		boolean isDiff = isDifferent(comparisonMap, dataListType, pivotKey, property, value);
		addComparisonDataItem(comparisonMap, dataListType, charactName, pivotKey, property, (!swap ? value : null), (!swap ? null : value),
				nbEntities, comparisonPosition, isDiff);
	}

	private String getCurrentValue(Map<String, CompareResultDataItem> comparisonMap, QName dataListType, String pivotKey, QName property,
	...
	private boolean isDifferent(Map<String, CompareResultDataItem> comparisonMap, QName dataListType, String pivotKey, QName property, String value) {
		String key = String.format("%s-%s-%s", dataListType, pivotKey, property);
		CompareResultDataItem comparisonDataItem = comparisonMap.get(key);
	...
	public boolean applyTo(QName entityType) {
		return entityType.equals(PLMModel.TYPE_FINISHEDPRODUCT) || entityType.equals(PLMModel.TYPE_SEMIFINISHEDPRODUCT)
				|| entityType.equals(PLMModel.TYPE_LOGISTICUNIT);
	}
