...
public class ForbiddenIngMaxUnitPatch extends AbstractBeCPGPatch {
	...
	protected String applyInternal() throws Exception {
		apply(PLMModel.TYPE_FORBIDDENINGLIST);
	...
	private void apply(QName sourceType) {
		...
		BatchProcessor<NodeRef> batchProcessor = new BatchProcessor<>("ForbiddenIngMaxUnitPatch", transactionService.getRetryingTransactionHelper(),
		...
		BatchProcessWorker<NodeRef> worker = new BatchProcessWorker<>() {
			...
			public void process(NodeRef dataListNodeRef) throws Throwable {
				...
				if (nodeService.exists(dataListNodeRef)) {
					AuthenticationUtil.setAdminUserAsFullyAuthenticatedUser();
					String maxUnit = (String) nodeService.getProperty(dataListNodeRef, PLMModel.PROP_FIL_QTY_PERC_MAXI_UNIT);
					if (maxUnit == null) {
						nodeService.setProperty(dataListNodeRef, PLMModel.PROP_FIL_QTY_PERC_MAXI_UNIT, "%");
					}
				} else {
