...
<#escape x as jsonUtils.encodeJSONString(x)>
	...
	<#if postData.isTopicPost>
		...
		<#if postData.supplierAccountRef??>
		"supplierAccountRef":
		{
			"userName": "${postData.supplierAccountRef.userName}",
			"firstName": "${postData.supplierAccountRef.firstName}",
			"lastName": "${postData.supplierAccountRef.lastName}"
		},	
		</#if>
