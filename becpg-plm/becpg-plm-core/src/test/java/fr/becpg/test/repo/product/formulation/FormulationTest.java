		...
		logger.info("testFormulateCostAndNutOfProductInkgAndgAndmLAndm");
		   ...
		   transactionService.getRetryingTransactionHelper().doInTransaction(new RetryingTransactionCallback<NodeRef>(){
				...
				for(CostListDataItem costListDataItem : formulatedProduct.getCostList()){
					...
					logger.debug(trace);
					if(costListDataItem.getCost().equals(cost1)){
						assertEquals("check cost", df.format(0.02d), df.format(costListDataItem.getValue()));
						assertEquals("check cost", df.format(0.402d), df.format(costListDataItem.getValuePerProduct()));
						assertEquals("check cost unit", "€/P", costListDataItem.getUnit());
						checks++;
					}
					if(costListDataItem.getCost().equals(cost2)){
						assertEquals("check cost", df.format(0.022d), df.format(costListDataItem.getValue()));
						assertEquals("check cost", df.format(0.444d), df.format(costListDataItem.getValuePerProduct()));
						assertEquals("check cost unit", "€/P", costListDataItem.getUnit());
						checks++;
					}
				}
