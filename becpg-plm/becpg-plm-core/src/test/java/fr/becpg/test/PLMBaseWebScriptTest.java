 ...
 * @author Matthieu
 */
public abstract class PLMBaseWebScriptTest extends PLMBaseTestCase {
	// Test Listener
	...
	public static class BaseWebScriptTestListener extends ResultPrinter implements WebScriptTestListener {
		 ...
		 * (non-Javadoc)
		 * 
		 * @see junit.textui.ResultPrinter#addError(junit.framework.Test,
		 * java.lang.Throwable)
		 */
		@Override
		public void addError(Test test, Throwable t) {
			getWriter().println("*** Error: " + ((PLMBaseWebScriptTest) test).getName());
		 ...
		 * (non-Javadoc)
		 * 
		 * @see junit.textui.ResultPrinter#addFailure(junit.framework.Test,
		 * junit.framework.AssertionFailedError)
		 */
		@Override
		public void addFailure(Test test, AssertionFailedError t) {
			getWriter().println("*** Failed: " + ((PLMBaseWebScriptTest) test).getName());
			t.printStackTrace(getWriter());
		}
		...
		public void endTest(Test test) {
			getWriter().println();
			getWriter().println("*** Test completed: " + ((PLMBaseWebScriptTest) test).getName());
		}
		...
		public void startTest(Test test) {
			getWriter().println();
			getWriter().println("*** Test started: " + ((PLMBaseWebScriptTest) test).getName() + " (remote: " + (((PLMBaseWebScriptTest) test).getRemoteServer() != null));
		}
	...
	/**
	 * Returns the custom {@link ServletAuthenticatorFactory} which is injected
	 * into an {@link TestWebScriptServer} instances that are returned, if any.
	...
	/**
	 * Sets that a custom {@link ServletAuthenticatorFactory} should be injected
	 * into instances of {@link TestWebScriptServer} returned from
	 * {@link #getServer()}
	 */
	 ...
	 * {@link #setCustomContext(String) custom context}
	 */
	 ...
	 * @throws IOException
	 */
	protected Response sendRequest(Request req, int expectedStatus) throws IOException {
	 ...
	 * @throws IOException
	 */
	protected Response sendRequest(Request req, int expectedStatus, String asUser) throws IOException {
		...
		if (traceReqRes && isLogEnabled()) {
			...
			// {
			// JSONObject jsonRsp =
			// (JSONObject)JSONValue.parse(res.getContentAsString());
	...
	// private void toJSONString(JSONObject map, StringWriter writer) throws
	// IOException
	// {
	// if(map == null)
	// {
	 ...
	 * @throws IOException
	 */
	protected Response sendLocalRequest(final Request req, final int expectedStatus, String asUser) throws IOException {
		asUser = (asUser == null) ? defaultRunAs : asUser;

		TestWebScriptServer tws = getServer();
		if (AuthenticationUtil.isMtEnabled()) {
			// MT repository container requires non-none authentication (ie.
			// guest or higher)
			// If the servlet authenticator is still the default, substitute in
			// a custom one
			// (If they test has already changed the authenticator, then stay
			// with that)
